...
from patch.predownload import store_pre_download_helper
...
class Downloader(object):
        ...
        try:
            ...
            for patch_info in self.patchlist:
                ...
                zip_info = [
                 ...
                 zip_size,
                 zip_md5,
                 zip_rw_path, zip_orbit_path, patch_utils.PATCH_ZIP_DOWNLOAD_STATE_INIT, 0,
                 self.get_zip_src_url(patch_zip_pattern, patch_info.target_version, patch_info)]
    ...
    def download_patchlist(self, callback):
        ...
        if not self.downloader_agent.start_download(download_list, self.on_download_patch_list_callback, 0, is_patch=False):
    ...
    def analyze_patch_zip_info(self, patch_config_data):
        ...
        for patchlist_info in self.patchlist:
            ...
            if version_str in patch_zip_info:
                ...
                zip_size, zip_md5, low_zip_size, low_zip_md5 = (int(cnt_zip_info[0]), cnt_zip_info[1],
...
1233     150  LOAD_GLOBAL           1  'patch_utils'
...
1255     348  LOAD_FAST             9  'versioned_info_dict'
...
1256     369  LOAD_FAST             6  'version_file_info'
         372  POP_JUMP_IF_FALSE   391  'to 391'

1257     375  LOAD_FAST             8  'version_file_lst'
...
1270     493  LOAD_FAST             7  'version_file_version'
         496  LOAD_CONST            1  ''
         499  COMPARE_OP            4  '>'
         502  POP_JUMP_IF_FALSE   561  'to 561'

1271     505  LOAD_FAST             0  'self'
...
Parse error at or near `BINARY_SUBSCR' instruction at offset 9
    ...
    def do_generate_npk_download_list(self, unchecked_list, info_map):
        ...
        for npk_name in unchecked_list:
            ...
            if os.path.exists(rw_path):
                if npk_name not in self._pdfiles:
                    os.remove(rw_path)
                else:
                    continue
    ...
    def download_npkfiles(self, cb):
        ...
        print('[downloader] download npk files start:', download_list)
        if not download_list:
    ...
    def copy_pdfiles(self, cb):
        self.set_state(patch_utils.DST_COPY_PD_FILE)
        self.npk_check_callback = cb
        self._pdfiles = []
        download_list = list(self.npk_unchecked_list)
        print('[downloader] copy pdfiles start:', download_list)
        store_pre_download_helper.check_and_move_files(download_list, patch_path.get_neox_dir(), self._on_pdfiles_progress, self._on_copy_pdfiles_finish)
        process_info = {'CP_PDFILES_START': 'npk names:{}'.format(download_list)}
        self.patch_dctool.send_patch_process_info_info(process_info)

    def _on_pdfiles_progress(self, progress):
        self.analyze_prog = progress

    def _on_copy_pdfiles_finish(self, success_list, error_msg=None):
        self.analyze_prog = 1
        if error_msg:
            print('[Except] [downloader] copy pdfiles failed', error_msg)
        print('[downloader] copy pdfiles finished:', success_list)
        self._pdfiles = success_list
        self.put_npk_check_ret_cb(patch_utils.NPK_STATE_COPY_PD_FILE_FINISHED)
        process_info = {'CP_PDFILES_FINISH': 'npk names:{}'.format(success_list)}
        self.patch_dctool.send_patch_process_info_info(process_info)

    def clear_pdfiles(self):
        npk_info = patch_utils.read_npk_info()
        if npk_info:
            all_complete_npk_names = []
            ...
            all_complete_npk_names.extend(files_info.keys())
            all_complete_npk_names.extend(high_end_files_info.keys())
            all_complete_npk_names.extend(low_end_files_info.keys())
            store_pre_download_helper.clear_files(all_complete_npk_names, False)
