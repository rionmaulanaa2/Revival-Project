...
class VoiceMgr(Singleton):
    ...
    def upload_amr(self, amr_file):
        ...
        upload_key = 'file'
        if self.get_project_id() != 'g93':
            upload_key = 'upload'
        if six.PY2:
            from poster.encode import multipart_encode
            datagen, header = multipart_encode({upload_key: file_handle})
        else:
            import urllib3
            datagen, content_type = urllib3.encode_multipart_formdata({upload_key: (path, voice_data, 'application/octet-stream')})
        ...
        def callback(ret, url, args):
            ...
            if ret == None:
                ...
                if int(status) != 0:
                    print('[ERROR] upload_amr error.', ret)
                    self._is_upload_amr = False
                    return
    ...
    def get_url(self):
        if global_data.channel.is_europe_america_server():
            return 'https://sm-g93na-1s9l3dqf-prod-ws.nie.easebar.com:12004/xyq_service/xyq-voice'
        else:
            return self._url
