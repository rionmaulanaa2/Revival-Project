...
class ComCharacter(UnitCom):
    BIND_EVENT = {'E_CHAR_X_OFF': '_set_x_off',
       ...
       'E_ENABLE_HOTFIX_CHAR': 'enable_hotfix_char'
       }
    ...
    def __init__(self):
        ...
        self._is_not_block = True
        self._last_col_position = None
        self._is_hotfix_col_enable = True
        return
    ...
    def init_from_dict(self, unit_obj, bdict):
        ...
        if self.ev_g_is_avatar():
            ...
            if version.get_engine_version() in ('1.0.18110', '1.0.18112') or global_data.is_inner_server:
                global_data.game_mgr.get_fix_logic_timer().register(func=self.check_fix_engine_overlap_bug, interval=1, times=-1, mode=timer.LOGIC)
    ...
    def check_overlap_tick(self, *args):
        ...
        if global_data.game_mode and global_data.game_mode.is_mode_type(game_mode_const.GAME_MODE_SURVIVALS):
            ...
            if pass_time < SHOW_GM_UI_CD_TIME:
                return
        if self._real_walk_direction.is_zero and self._is_not_block or not self._last_foot_position:
        ...
        if is_hit:
            ...
            if not is_hit:
                if not self._touch_wall:
                    ...
                    self._keep_same_pos_start_time = now_time
                    self._last_stuck_time = 0
                    self._is_not_block = is_not_block
                    return
    ...
    def enable_hotfix_char(self, val):
        self._is_hotfix_col_enable = val

    def check_fix_engine_overlap_bug(self):
        if not self.is_valid():
            return timer.RELEASE
        else:
            if not self.char_ctrl:
                return timer.RELEASE
            scn = self.scene
            if not scn or not scn.scene_col:
                return timer.RELEASE
            if not self.char_ctrl.isActive() or not self._is_hotfix_col_enable:
                self._last_col_position = None
                return
            las_col_pos = self._last_col_position
            cur_col_pos = self.char_ctrl.physicalPosition
            if las_col_pos is None:
                self._last_col_position = cur_col_pos
                return
            dir_len = (cur_col_pos - las_col_pos).length_sqr
            if dir_len <= 0.1:
                return
            if dir_len > 16900:
                self._last_col_position = cur_col_pos
                return
            ret = scn.scene_col.hit_by_ray(las_col_pos, cur_col_pos, 0, GROUP_CHARACTER_INCLUDE, GROUP_CHARACTER_INCLUDE, collision.INCLUDE_FILTER)
            if ret[0]:
                self._set_phys_pos_ignore_test(las_col_pos)
            else:
                self._last_col_position = cur_col_pos
            return
...
1334      12  LOAD_CONST            2  0.1
          15  LOAD_GLOBAL           1  'NEOX_UNIT_SCALE'
          18  BINARY_MULTIPLY  
          19  STORE_FAST            3  'x'

1335      22  LOAD_CONST            3  0.9
          25  LOAD_GLOBAL           1  'NEOX_UNIT_SCALE'
          28  BINARY_MULTIPLY  
          29  STORE_FAST            4  'y'

1336      32  LOAD_CONST            4  0.5
          35  LOAD_GLOBAL           1  'NEOX_UNIT_SCALE'
          38  BINARY_MULTIPLY  
          39  STORE_FAST            5  'z'

1337      42  LOAD_GLOBAL           2  'math3d'
