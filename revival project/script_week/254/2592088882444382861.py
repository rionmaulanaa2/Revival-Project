...
class impCustomRoom(object):
    ...
    def req_set_room_faction(self, faction_id):
        self.call_server_method('req_set_room_faction', (faction_id,))

    @rpc_method(CLIENT_STUB, (Int('uid'), Int('faction_id')))
    def on_req_set_room_faction(self, uid, faction_id):
        self.room_info.update_player_info_by_uid(uid, {'custom_faction_id': faction_id})
        room_ui = global_data.ui_mgr.get_ui('RoomUINew')
        if room_ui:
            room_ui.on_req_set_room_faction(uid, faction_id)

    def judge_set_room_faction(self, uid, faction_id):
        self.call_server_method('judge_set_room_faction', (uid, faction_id))

    def set_can_change_faction(self):
        self.call_server_method('set_can_change_faction', ())

    @rpc_method(CLIENT_STUB, (Bool('change_faction'),))
    def on_set_can_change_faction(self, change_faction):
        if change_faction:
            global_data.game_mgr.show_tip(get_text_by_id(860467), True)
        else:
            global_data.game_mgr.show_tip(get_text_by_id(860468), True)
        self.room_info.update_room_info_by_key_value('can_change_faction', change_faction)
        global_data.emgr.set_can_change_faction_event.emit(change_faction)
    ...
    def room_can_change_faction(self):
        if not self.room_info:
            return False
        return self.room_info._room_info.get('can_change_faction', False)

    def is_custom_faction_room(self):
        from logic.gcommon.common_const import battle_const
        return self.room_info.get_battle_type() == battle_const.FACTION_ROOM_BATTLE_TYPE

    def is_room_creator(self, uid=None):
        creator_uid = self.room_info._room_info.get('creator', 0)
        if uid is None:
            return self.uid == creator_uid
        else:
