...
class LotteryArtCollectionWidgetNew(LotteryBaseWidget):
    ...
    def init_parameters(self):
        ...
        if loop_lottery_utils.is_loop_lottery(self.lottery_id):
            ...
            loop_lottery_utils.fill_loop_lottery_info(self._conf, self.lottery_id)
        else:
            self._conf = conf
        ...
        self._request_activity_timestamp = 0
        return
    ...
    def init_temporary_activity_entrance(self):
        self.activity_button_widget = ArtCollectionActivityEntryWidget(self.panel, self.panel, self.data.get('activity_type'), self.data.get('remind_jump_ui')[0], self.data.get('remind_jump_ui')[1], lottery_id=self.lottery_id)
        if loop_lottery_utils.is_loop_lottery(self.lottery_id):
            if tutil.time() - self._request_activity_timestamp > 5:
                self._request_activity_timestamp = tutil.time()
    ...
    def _init_time_label(self):
        ...
        if self.is_visible_close:
            if loop_lottery_utils.is_loop_lottery(self.lottery_id):
                ...
                now = tutil.time()
                if shop_open_info:
                    left_time = shop_open_info[2] - now
                self.remain_time = int(left_time)
            else:
                self.remain_time = int(self.visible_ts[1] - tutil.get_server_time())
        elif loop_lottery_utils.is_loop_lottery(self.lottery_id):
            goods_open_info, shop_open_info = loop_lottery_utils.get_loop_lottery_open_info(self.lottery_id)
            left_time = 0
            now = tutil.time()
            if goods_open_info:
                left_time = goods_open_info[2] - now
            self.remain_time = int(left_time)
        else:
    ...
    def _set_preview_widget_visible(self, is_visible, default_tab_index=None, init_goods_id=None, force_refresh=False):
        ...
        if loop_lottery_utils.is_loop_lottery(self.lottery_id) and not loop_lottery_utils.is_loop_lottery_open(self.lottery_id):
            default_tab_index = REVIEW_TAB_INDEX
    ...
    def get_lottery_rule(self):
        if loop_lottery_utils.is_loop_lottery(self.lottery_id):
            ...
            if goods_open_info:
                ...
                lottery_start = shop_open_info[1]
                lottery_end = shop_open_info[2] - tutil.ONE_WEEK_SECONDS
            content = content[0]
            content_text_id = content[0]
            content_args = content[1]
            ...
            item_str = get_text_by_id(content_args.get('item'))
            content_str = get_text_by_id(content_text_id).format(time=time_str, item=item_str)
            return (
             title, content_str)
        else:
            title, content = self.data.get('rule_desc', [608106, 608107])
