...
from logic.gutils.team_utils import get_teammate_colors, get_factionmate_colors
...
class ScalePlateBaseUI(BasePanel):
    ...
    def _add_scale_mark(self, player_id, mark_pos, type, is_init):
        ...
        if global_data.battle and global_data.battle.is_custom_faction_room:
            if group_ids:
                mark_color = get_factionmate_colors(group_ids)[player_id] if 1 else MAP_COL_GREEN
            else:
                mark_color = get_teammate_colors(group_ids)[player_id] if group_ids else MAP_COL_BLUE
