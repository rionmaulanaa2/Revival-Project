...
class OrbitDownloader(object):
    ...
    def check_http_url(self, download_list):
        for item in download_list:
            url = item[2]
            ret = url.startswith('http://')
            if ret:
                return True
    ...
    def init_download_env(self, download_list=[], total_size=0, cb=None, override=True, is_patch_mode=True):
        self.download_map = {}
        ...
        if not ORBIT_V3 and self.check_ip_url(self.download_list):
            self.cnt_orbit_config['notusecdn'] = 'true'
        if override:
            self.cnt_orbit_config['isrenew'] = 'true'
        if ORBIT_V3:
            if self.check_http_url(self.download_list):
                self.cnt_orbit_config['type'] = 'other'
            elif is_patch_mode:
                self.cnt_orbit_config['type'] = 'patch'
            else:
                self.cnt_orbit_config['type'] = 'list'
    ...
    def orbit_download_callback(self, data, result, complete):
        ...
        if code != 0:
            print('[ORBIT DOWNLOAD ERROR] {} callback data is {}'.format('V3' if ORBIT_V3 else 'V2', data))
    ...
    def start_download(self, download_list, cb, total_size=0, override=True, is_patch=True):
        ...
        if self.downloading:
            ...
            return False
        self.init_download_env(download_list, total_size, cb, override, is_patch)
