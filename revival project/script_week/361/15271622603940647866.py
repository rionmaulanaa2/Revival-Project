...
from logic.gutils.custom_room_utils import get_room_layout_cls, RANDOM_MAP_TEXT_ID, get_custom_faction_config
...
class RoomUINew(BasePanel):
    ...
    UI_ACTION_EVENT = {'btn_dissolve.btn_major.OnClick': 'on_click_dissolve_btn',
       ...
       'temp_lock_faction.btn.OnClick': 'on_click_enable_change_faction',
    ...
    GLOBAL_EVENT = {'set_can_change_faction_event': 'on_set_can_change_faction'
       }
    ...
    def on_init_panel(self, room_info):
        ...
        self.has_init_faction_list = False
    ...
    def init_widget(self):
        ...
        if global_data.player.uid == self.room_info.creator:
            ...
            is_custom_faction_room = self.is_custom_faction_room(self.room_info.battle_type)
            forbid_mode_choose = is_custom_faction_room
            ...
            self.panel.nd_owner.btn_map_choose.setVisible(is_creator and not forbid_mode_choose)
            self.panel.nd_player.img_map_bar.setVisible(not is_creator or forbid_mode_choose)
            self.panel.nd_owner.btn_mode_choose.setVisible(is_creator and not forbid_mode_choose)
            self.panel.nd_player.img_mode_bar.setVisible(not is_creator or forbid_mode_choose)
            ...
            self.panel.btn_faction_choose.setVisible(is_custom_faction_room)
            self.panel.faction_list.setVisible(is_custom_faction_room)
    ...
    def init_custom_room_data(self):
        ...
        if not is_competition:
            for battle_type, mode_conf in six.iteritems(battle_config):
                if self.is_room_forbid_mode_choose(battle_type):
                    continue
    ...
    def init_faction_list(self):
        if not self.room_info:
            return
        if not self.is_custom_faction_room(self.room_info.battle_type):
            return
        player_info = self.room_info.get_player_data(global_data.player.uid)
        if not player_info:
            return
        self.panel.btn_faction_choose.setVisible(not self.is_judgement())
        self.panel.faction_list.setVisible(False)
        if self.has_init_faction_list:
            return
        self.has_init_faction_list = True
        if self.is_judgement():
            return
        custom_faction_config = get_custom_faction_config(self.room_info.battle_type)
        faction_ids = sorted(list(custom_faction_config.keys()))
        self.panel.btn_faction_choose.setVisible(True)
        self.panel.faction_list.setVisible(False)
        icon_arrow = self.panel.btn_faction_choose.img_icon

        @self.panel.btn_faction_choose.unique_callback()
        def OnClick(btn, touch):
            visible = self.panel.faction_list.isVisible()
            if not visible and not global_data.player.room_can_change_faction():
                global_data.game_mgr.show_tip(get_text_by_id(860468), True)
                return
            self.panel.faction_list.setVisible(not visible)
            rotation = 0 if visible else 180
            icon_arrow.setRotation(rotation)

        faction_option = [ {'name': custom_faction_config[faction_id],'faction_id': faction_id} for faction_id in faction_ids ]

        def call_back(index):
            selected_faction = faction_option[index]['faction_id']
            global_data.player.req_set_room_faction(int(selected_faction))
            self.panel.btn_faction_choose.SetText(faction_option[index]['name'])
            self.panel.faction_list.setVisible(False)
            icon_arrow.setRotation(0)

        template_utils.init_common_choose_list_2(self.panel.faction_list, icon_arrow, faction_option, call_back, max_height=354)
        cur_faction_id = player_info.get('custom_faction_id', -1)
        self.panel.btn_faction_choose.SetText(custom_faction_config.get(cur_faction_id, ''))

    def update_selected_faction(self):
        if not self.room_info:
            return
        if not self.is_custom_faction_room(self.room_info.battle_type):
            return
    ...
    def init_can_chat_room_widget(self):
        if self.room_info:
            show_lock_voice_widget = self.is_competition_room()
    ...
    def init_can_change_faction_widget(self):
        player = global_data.player
        if not player:
            return
        show_can_change_faction_widget = player.is_custom_faction_room()
        show_choose = show_can_change_faction_widget and player.room_can_change_faction()
        self.panel.temp_lock_faction.setVisible(show_can_change_faction_widget)
        self.panel.temp_lock_faction.choose.setVisible(show_choose)
        self.panel.temp_lock_faction.text.SetString(860466)
    ...
    def populate_judge_seat_ui(self, seat_ui, seat_idx, judge_data=None):
        if seat_ui is None:
            ...
            if self._is_week_competition:
                if global_data.player.uid not in self.room_info.fixed_ob_uids:
    ...
    def show_player_brief_info(self, seat_ui, player_id, seat_idx):
        ...
        if self.room_info.creator == global_data.player.uid:
            ...
            if self._is_week_competition and global_data.player.uid in self.room_info.fixed_ob_uids:
                show_btns = [
                 BTN_TYPE_ROOM_KICK_OUT]
            else:
                show_btns = []
    ...
    def on_self_judegement_change(self, become_judgement):
        ...
        if self.is_competition(self.room_info.battle_type):
            ...
            if not self.is_room_forbid_mode_choose(self.room_info.battle_type):
    ...
    def on_set_can_change_faction(self, change_faction):
        self.panel.temp_lock_faction.choose.setVisible(change_faction)
    ...
    def on_req_set_room_faction(self, uid, faction_id):
        self.room_info and self.room_info.update_roommate_info(uid, {'custom_faction_id': faction_id})
        self.team_layout.on_req_set_room_faction(uid, faction_id)
        if uid == global_data.player.uid:
            self.update_selected_faction()
    ...
    def on_click_enable_change_faction(self, *args):
        player = global_data.player
        if not player or not player.is_room_creator():
            return
        global_data.player.set_can_change_faction()
    ...
    def set_appear_effect(self):
        self.hide_main_ui(exceptions=('MainChat', 'JudgeChooseFactionUI'))
        ui = global_data.ui_mgr.get_ui('MainChat')
    ...
    def is_room_forbid_mode_choose(self, battle_type):
        if int(battle_type) == battle_const.FACTION_ROOM_BATTLE_TYPE:
            return True
        return False

    def is_custom_faction_room(self, battle_type):
        if int(battle_type) == battle_const.FACTION_ROOM_BATTLE_TYPE:
            return True
        return False
