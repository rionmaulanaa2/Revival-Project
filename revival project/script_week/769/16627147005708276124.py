...
class RoomCreateUINew(WindowMediumBase):
    ...
    def _can_create_room(self, battle_type):
        ...
        if needed_item_no is None:
            ...
            has_competition_card = global_data.player.has_item_by_no(iconst.ITEM_NO_ROOM_COMPETITION_CARD)
            has_faction_room_card = global_data.player.has_item_by_no(iconst.ITEM_NO_ROOM_FACITON_ROOM_CARD)
            if int(battle_type) == battle_const.FACTION_ROOM_BATTLE_TYPE:
                return has_competition_card and has_faction_room_card
            return has_competition_card
            return
