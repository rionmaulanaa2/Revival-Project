...
class ComGroup(UnitCom):
    ...
    def __init__(self):
        ...
        self._is_login_faction_ccmini = False
    ...
    def login_faction_ccmini(self):
        battle = self.battle
        if not battle or not battle.is_custom_faction_room:
            return
        if not self._is_login_faction_ccmini and self.unit_obj == global_data.player.logic:
            global_data.player.req_near_login_session_data()
            self._is_login_faction_ccmini = True
    ...
    def destroy(self):
        if self._is_login_team_ccmini:
            global_data.ccmini_mgr.logout_session(const.TEAM_ALL_SESSION_ID)
            self._is_login_team_ccmini = False
        if self._is_login_faction_ccmini:
