...
CN_IOS_SHIPPING_VER = 18197
NA_IOS_SHIPPING_VER = 18257
...
class MainSettingUI(BasePanel):
    ...
    def btn_gift_show_check(self):
        ...
        engine_last_ver = int(engine_ver.split('.')[-1])
        if game3d.get_platform() == game3d.PLATFORM_IOS and game3d.is_release_version():
            max_show_ver = G_IS_NA_USER or CN_IOS_SHIPPING_VER if 1 else NA_IOS_SHIPPING_VER
            if engine_last_ver > max_show_ver:
                return False
    ...
    def open_privacy_info_url(self, prefix):
        ...
        if game3d.get_platform() == game3d.PLATFORM_IOS:
            safeurl_base64_data = six.ensure_str(base64.urlsafe_b64encode(six.ensure_binary(dict_json))).rstrip('=')
        else:
            safeurl_base64_data = six.ensure_str(base64.urlsafe_b64encode(six.ensure_binary(dict_json)))
        url = prefix + safeurl_base64_data
        if global_data.channel:
