...
class TrioSwitchWeapon(StateBase):
    ...
    def init_from_dict(self, unit_obj, bdict, sid, info):
        from logic.gcommon.common_const.ui_operation_const import WEAPON_FAST_SWITCH_8009
        ...
        self._is_fast_switch = False
        self._fast_switch_enable = global_data.player.get_setting_2(WEAPON_FAST_SWITCH_8009)
        global_data.emgr.update_weapon_fast_switch_8009 += self.on_setting_changed
        self.enable_param_changed_by_buff()
        return

    def destroy(self):
        super(TrioSwitchWeapon, self).destroy()
        global_data.emgr.update_weapon_fast_switch_8009 -= self.on_setting_changed

    def on_setting_changed(self, enable):
        self._fast_switch_enable = enable
    ...
    def action_btn_double_click(self):
        if self.is_active and self._fast_switch_enable:
            self._is_fast_switch = True
        super(TrioSwitchWeapon, self).action_btn_double_click()
        return True
    ...
    def enter(self, leave_states):
        ...
        def delay_func():
            if self.is_active:
                self.send_event('E_DO_SKILL', self.skill_id, self._is_fast_switch)

        global_data.game_mgr.delay_exec(0.3, delay_func)
        return
    ...
    def on_take_out(self):
        take_out_anim_change = {'8009': 'switch_nachubuqiang',
           '8009_S': 'switch_nachuxiandanqiang',
           '8009_R': 'switch_nachudaodan'
           }
        self.take_out_anim = take_out_anim_change.get(self.to_switch_id, self.take_out_anim)
