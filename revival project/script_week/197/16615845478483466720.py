...
from patch.predownload import hotfix_tempcode
RE_TRY_TIMES = 10000
...
class ExtInGameManager(Singleton):
    ...
    def ext_npk_config_analyze(self, check_all=True):
        ...
        self._clear_pdfiles(downloaded_exts)
    ...
    def download_ext_npk(self):
        ...
        if not self._npk_download_list or met_error:
            ...
            self._delay_retry_analyze()
            return
        self._check_pdfiles_and_download_ext_npk()

    def _check_pdfiles_and_download_ext_npk(self):
        npk_download_list = self._npk_download_list
        pdfiles_list = []
        for ext_dl_list, ext_name in npk_download_list:
            for orbit_path, rw_path, dl_url, npk_size, npk_md5 in ext_dl_list:
                pdfiles_list.append(os.path.basename(rw_path))

        target_dir = ext_package_utils.get_extend_npk_dir()

        def _on_copy_pdfiles_finish(success_list, err_msg=None):
            if success_list:
                success_set = set(success_list)
                for _ext_dl_list, _ext_name in self._npk_download_list:
                    new_ext_dl_list = []
                    for _orbit_path, _rw_path, _dl_url, _npk_size, _npk_md5 in ext_dl_list:
                        if os.path.basename(rw_path) in success_set:
                            continue
                        new_ext_dl_list.append((_orbit_path, _rw_path, _dl_url, _npk_size, _npk_md5))

                    if len(new_ext_dl_list) != len(ext_dl_list):
                        ext_dl_list[:] = []
                        ext_dl_list.extend(new_ext_dl_list)

                store_pre_download_helper.clear_files(list(success_list), False)
            self._download_ext_npk()

        store_pre_download_helper.check_and_move_files(pdfiles_list, target_dir, None, _on_copy_pdfiles_finish)
        return

    def _clear_pdfiles(self, downloaded_exts):
        auto_download_finished = True
        for ext_name in self._ext_package_config:
            auto_dl = self._ext_package_config.get(ext_name, {}).get('auto_download', 1)
            if auto_dl and ext_name not in downloaded_exts:
                auto_download_finished = False
                break

        if auto_download_finished:
            store_pre_download_helper.clear_files([], True)

    def _download_ext_npk(self):
        self._all_npk_size = 0
