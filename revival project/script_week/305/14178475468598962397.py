...
class ILoadingInterface(object):
    ...
    def update_percent(self, value):
        raise RuntimeError('loading widget must implement interface [update_percent]')

    def loading_init(self):
        raise RuntimeError('loading widget must implement interface [loading_init]')

    def loading_end(self):
        raise RuntimeError('loading widget must implement interface [loading_end]')

    def loading_update(self, dt):
        raise RuntimeError('loading widget must implement interface [loading_update]')
