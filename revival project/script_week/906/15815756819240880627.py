...
def get_login_video_by_time():
    import time
    timestamp = int(time.time())
    video_list = confmgr.get('login_video_list')
    for item in reversed(video_list):
        if item.get('begin_time') < timestamp:
            return 'video/{}.mp4'.format(item.get('video'))
...
def get_login_video_name():
    import C_file
    video_path = get_login_video_by_time()
