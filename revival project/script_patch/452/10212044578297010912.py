...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/entities/GranhackSurvivalBattle.py
...
class GranhackSurvivalBattle(SurvivalBattle):

    def __init__(self, entityid):
        super(GranhackSurvivalBattle, self).__init__(entityid)
        global_data.emgr.resolution_changed += self.on_resolution_changed

    def init_from_dict(self, bdict):
        self.reconnect_handle_data(bdict)
        super(GranhackSurvivalBattle, self).init_from_dict(bdict)
        self._show_tip_cache = False
        self._timestamp_cache = 0
        self._cache_valid = False

    def load_finish(self):
        super(GranhackSurvivalBattle, self).load_finish()
    ...
    def granbelm_teleport_single(self, entity_id, portal_type, tele_role, tele_stage, below_pos, tele_tag):
        ...
        if global_data.cam_lplayer.id == entity_id:
            if tele_stage == GRANBELM_PORTAL_FIRST_TELEPORT:
                ...
                global_data.sound_mgr.play_sound_2d('Play_ui_notice', ('ui_notice', granhack_utils.get_tele_sound_map(portal_type, 0)))
                global_data.emgr.scene_stop_poison_circle.emit()
                global_data.gran_sur_battle_mgr.set_tele_tag(tele_tag)
                if granhack_utils.check_switch_model_visible():
