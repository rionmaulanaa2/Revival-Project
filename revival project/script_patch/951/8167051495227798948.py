...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivitySakuganPuzzle.py
...
class ActivitySakuganPuzzle(ActivityTemplate):
    TASK_TEMPLATE = 'activity/activity_202111/i_activity_sakugan_puzzle_task'
    ...
    def on_init_panel(self):
        ...
        self.task_panel.nd_task_list.btn_close.BindMethod('OnClick', lambda *args: self.task_panel.setVisible(False))
        ...
        init_tempate_mall_i_item(self.panel.temp_fragment, self.ui_data['coin_id'], 6, show_tips=True)
        ...
        self.panel.runAction(cc.Sequence.create([
         ...
         cc.CallFunc.create(lambda : self.panel.PlayAnimation('loop_03'))]))
        self.update_exchange_node()
    ...
    def update_task_dlg(self, *args):
        ...
        random_refresh_type = get_task_fresh_type(self.ui_data['daily_random'])
        children_task_list = []
        random_task_list = global_data.player.get_random_children_tasks(random_refresh_type, self.ui_data['daily_random'])
        fixed_task_list = task_utils.get_children_task(task_id)
        if random_task_list is None:
            log_error('ActivitySakuganPuzzle no random children tasks!')
            random_task_list = []
    ...
    def on_cover_unlocked(self, idx):
        ...
        if self.check_all_unlocked():
            ...
            act = [cc.CallFunc.create(lambda : item.PlayAnimation('click')),
             cc.DelayTime.create(item.GetAnimationMaxRunTime('click')),
             cc.CallFunc.create(lambda : self.update_exchange_node()),
             ...
             cc.CallFunc.create(lambda : self.get_reward_from_cache(reason)),
             cc.CallFunc.create(lambda : self.get_reward_from_cache('OPEN_ALL_PUZZLE'))]
        else:
            act = [cc.CallFunc.create(lambda : item.PlayAnimation('click')),
             cc.DelayTime.create(item.GetAnimationMaxRunTime('click')),
             cc.CallFunc.create(lambda : self.get_reward_from_cache(reason))]
    ...
    def get_reward_from_cache(self, reason):
        if self.check_all_unlocked():
            global_data.ui_mgr.close_ui('ScreenLockerUI')
        global_data.emgr.show_cache_specific_reward.emit(reason)
        self.update_key_count()
        self.update_prog()
        self.update_exchange_node()

    def on_finalize_panel(self):
        super(ActivitySakuganPuzzle, self).on_finalize_panel()
    ...
    def show_tab_rp(activity_id):
        if ActivitySakuganPuzzle.show_task_rp(activity_id):
            return True
        else:
            activity_conf = confmgr.get('c_activity_config', activity_id)
            puzzle_finish = len(global_data.player.opened_puzzle_data) >= PUZZLE_COLUMN_CNT * PUZZLE_ROW_CNT
            if puzzle_finish:
                return ActivitySakuganPuzzle.show_exchange_rp(activity_id)
            ui_data = activity_conf['cUiData']
