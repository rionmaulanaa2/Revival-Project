...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/lobby/EntryWidget/LobbySpringFestivalEntryWidget.py
...
class LobbySpringFestivalEntryWidget(LobbyEntryWidgetBase):
    GLOBAL_EVENT = {}

    def on_init_widget(self):
        super(LobbySpringFestivalEntryWidget, self).on_init_widget()

        @self.panel.btn.unique_callback()
        def OnClick(*args):
            activity_list = self.get_activity_list()
            if not activity_list:
                global_data.game_mgr.show_tip(601213)
    ...
    def on_finalize_widget(self):
        super(LobbySpringFestivalEntryWidget, self).on_finalize_widget()
