...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/common_ui/RealNameRegisterUI.py
...
class RealNameRegisterUI(BasePanel):
    ...
    def close(self):
        if self.close_cb:
            self.close_cb()
        super(RealNameRegisterUI, self).close()
    ...
    def init_widget(self):
        ...
        template_utils.init_common_panel(self.panel.tenp_bg, None, on_close=self.close)
    ...
    def init_country_list(self):
        ...
        template_utils.init_common_choose_list(self.panel.nation_list, country_list, callback=choose, max_height=600)
    ...
    def on_click_register(self, btn, touch):
        ...
        if not name:
            NormalConfirmUI2().set_content_string(get_text_by_id(81987))
            return
        if not id_num:
            NormalConfirmUI2().set_content_string(get_text_by_id(81986))
            return
        if self._is_requesting_realname_result:
            global_data.game_mgr.show_tip(609400)
            return
        self.confirm_cb(name, id_num, self.region_id, callback=self.realname_ret)
    ...
    def realname_ret(self, success, message):
        ...
        NormalConfirmUI2(on_confirm=callback).set_content_string(message)