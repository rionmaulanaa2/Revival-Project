...
class SvrSelectUI(BasePanel):
    ...
    def update_recommend_servers(self):
        rec_list = self.recommend_list
        rec_list.DeleteAllSubItem()
        rec_svr_data_list = LoginHelper().registed_server_list
        if rec_svr_data_list:
            if len(rec_svr_data_list) > 6:
                rec_svr_data_list = rec_svr_data_list[:6]
            self.add_list_header(get_text_by_id(2141), rec_list)
            server_list_item = global_data.uisystem.load_template_create('login/server_list_item')
            self.update_server_list(server_list_item.list_container, rec_svr_data_list, True)
            list_container = server_list_item.list_container
            list_container.removeFromParent()
            rec_list.AddControl(list_container, bRefresh=True)
        elif LoginHelper().account_state == LoginHelper.SERVER_LIST_STATE_CONNECTING:
            self.add_list_header(get_text_by_id(2142), rec_list, 'login/server_waiting')
        self.add_list_header(get_text_by_id(2143), rec_list)
        rec_svr_data_list = LoginHelper().recommend_server_list
        if rec_svr_data_list:
            server_list_item = global_data.uisystem.load_template_create('login/server_list_item')
            self.update_server_list(server_list_item.list_container, rec_svr_data_list, False)
            list_container = server_list_item.list_container
            list_container.removeFromParent()
            rec_list.AddControl(list_container, bRefresh=True)
