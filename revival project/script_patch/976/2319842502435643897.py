...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/gcommon/component/client/com_pet/ComPetBehavior.py
import six
import copy
import game3d
...
import logic.gcommon.cdata.pet_status_config as pet_status_config
...
class ComPetBehavior(UnitCom):
    ...
    def __init__(self):
        super(ComPetBehavior, self).__init__()
    ...
    def init_from_dict(self, unit_obj, bdict):
        super(ComPetBehavior, self).init_from_dict(unit_obj, bdict)
        self._bdict = bdict
        if self.sd.ref_is_mecha or self.unit_obj.is_monster():
            self.status_config = mecha_status_config
        elif self.unit_obj.is_pet():
            self.status_config = pet_status_config
        else:
        ...
        self._default_state = bdict.get('default_state', None)
        if self._default_state is None:
            self._default_state = self.status_config.PT_IDLE
    ...
    def on_init_complete(self):
        ...
        self.detect_const()
        self.on_active_state(self.status_config.PT_IDLE)
    ...
    def destroy(self):
        super(ComPetBehavior, self).destroy()
    ...
    def reset_state_param(self, re_sound=True, include_camera_param=False):
        behavior_info = self.get_behavior_config()
        new_param_info = self.get_param_config()
    ...
    def update_state(self, dt):
        ...
        for state_id in target_state:
            if state_id not in self._states:
                log_error('Invalid mecha state_id {0}!!!!!!!!!!!!!!!!!!!!'.format(state_id))
                continue
            self.ev_g_trans_status(state_id)
    ...
    def on_load_animator_complete(self, *args):
        ...
        self.set_default_state(PT_IDLE)
    ...
    def on_enable_behavior(self, force=False):
        self.need_update = self.ev_g_is_my_pet()
