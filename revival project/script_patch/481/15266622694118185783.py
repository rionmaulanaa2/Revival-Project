...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/battle/Assault/AssaultTopScoreUI.py
...
class AssaultTopScoreUI(BasePanel):
    ...
    def update_group_score(self, data):
        ...
        if global_data.cam_lplayer:
            join_point = bat.get_enter_group_data(global_data.cam_lplayer.id)
        for g_id in six.iterkeys(data):
            group_point = data[g_id] - join_point.get(int(g_id), 0)
            if g_id == self.group_id:
                blue_score = self.panel.nd_score.lab_score_blue.getString()
                self.panel.nd_score.lab_score_blue.SetString(str(group_point))
                if blue_score != str(group_point):
                    self.panel.StopAnimation('blue_up')
                    self.panel.PlayAnimation('blue_up')
                self.panel.prog_blue.setVisible(False)
            else:
                red_score = self.panel.nd_score.lab_score_red.getString()
                self.panel.nd_score.lab_score_red.SetString(str(group_point))
                if red_score != str(group_point):
                    self.panel.StopAnimation('red_up')
                    self.panel.PlayAnimation('red_up')
                self.panel.prog_red.setVisible(False)
