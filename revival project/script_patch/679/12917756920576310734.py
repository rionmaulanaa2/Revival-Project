...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/gcommon/component/client/ComTriggerGrenadeAppearance.py
...
from logic.gcommon.common_const.collision_const import GROUP_STATIC_SHOOTUNIT, GROUP_STATIC_SHOOTUNIT, REGION_SCENE_GROUP
...
MODEL_DEFAULT_FORWARD = math3d.vector(1, 0, 0)

class ComTriggerGrenadeAppearance(ComBaseModelAppearance):
    ...
    def __init__(self):
        super(ComTriggerGrenadeAppearance, self).__init__()
        self.itype = None
    ...
    def init_from_dict(self, unit_obj, bdict):
        ...
        super(ComTriggerGrenadeAppearance, self).init_from_dict(unit_obj, bdict)
        forward = bdict.get('forward', [0, 0, 1])
        if forward is None:
            forward = [
             0, 0, 1]
        self.forward = math3d.vector(*forward)
        self._owner_mecha_fashion_id = bdict.get('owner_fashion_id', DEFAULT_CLOTHING_ID)
        self._owner_mecha_shiny_weapon_id = bdict.get('owner_shiny_weapon_id', None)
        return
    ...
    def _get_empty_model_path(self):
        model_path = get_explosive_robot_conf_val(self.itype, 'model', default='model_new/mecha/8023/8023/tower/shoulei_1.gim', skin_id=self._owner_mecha_shiny_weapon_id or self._owner_mecha_fashion_id)
        model_path = model_path.split(',')
        if len(model_path) > 1:
            if self.ev_g_is_enemy():
                return model_path[0]
            return model_path[1]
        return model_path[0]
    ...
    def on_load_model_complete(self, model, userdata):
        ...
        scale = 0.2
        if self.itype:
            scale = get_explosive_robot_conf_val(self.itype, 'model_scale', default=1.0, skin_id=self._owner_mecha_shiny_weapon_id or self._owner_mecha_fashion_id)
        model.scale = math3d.vector(scale, scale, scale)
        self.init_lod()
        is_enemy = self.ev_g_is_enemy()
        sfx = model.get_socket_obj('fx_red')
        if sfx and sfx.valid:
            sfx.visible = is_enemy
        sfx = model.get_socket_obj('fx_blue')
        if sfx and sfx.valid:
            sfx.visible = not is_enemy
        if global_data.game_mode and global_data.game_mode.is_mode_type(game_mode_const.TriggerGrenade_OutLine):
            if is_enemy and not self._is_outline:
        ...
        result = self.scene.scene_col.hit_by_ray(position + math3d.vector(0, 5, 0), position + math3d.vector(0, -5, 0), 0, GROUP_STATIC_SHOOTUNIT | REGION_SCENE_GROUP, GROUP_STATIC_SHOOTUNIT | REGION_SCENE_GROUP, collision.INCLUDE_FILTER, False)
        if result[0]:
            model.rotation_matrix = math3d.matrix.make_rotation_between(MODEL_DEFAULT_FORWARD, result[2])
        else:
            result = self.scene.scene_col.hit_by_ray(position - self.forward * 5, position + self.forward * 5, 0, GROUP_STATIC_SHOOTUNIT | REGION_SCENE_GROUP, GROUP_STATIC_SHOOTUNIT | REGION_SCENE_GROUP, collision.INCLUDE_FILTER, False)
            if result[0]:
                if (result[2] + MODEL_DEFAULT_FORWARD).length < 0.01:
                    model.rotation_matrix = math3d.matrix.make_rotation_y(math.pi)
                else:
                    model.rotation_matrix = math3d.matrix.make_rotation_between(MODEL_DEFAULT_FORWARD, result[2])
                model.position = result[1]
            else:
                model.rotation_matrix = math3d.matrix.make_rotation_between(MODEL_DEFAULT_FORWARD, math3d.vector(0, 1, 0))

    def destroy(self):
        if self._is_outline:
            self.send_event('E_DEL_MATERIAL_STATUS', 'ComCamp_always_outline')
            self._is_outline = False
        super(ComTriggerGrenadeAppearance, self).destroy()
