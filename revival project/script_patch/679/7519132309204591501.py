...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/mecha_display/MechaBasicInfoWidget.py
from __future__ import absolute_import
from common.uisys.BaseUIWidget import BaseUIWidget
from logic.comsys.mecha_display.MechaBasicInfoProficiencyWidget import MechaBasicInfoProficiencyWidget
from logic.comsys.mecha_display.MechaBasicSkinWidget import MechaBasicSkinWidget
from logic.comsys.mecha_display import MechaDetails

class MechaBasicInfoWidget(BaseUIWidget):
    USE_CNT_LIST = [
     1, 2, 5]

    def __init__(self, parent, panel):
        super(MechaBasicInfoWidget, self).__init__(parent, panel)
        self.init_parameters()
        self._basic_prof_widget = None
    ...
    def destroy(self):
        ...
        if self._basic_prof_widget:
            self._basic_prof_widget.destroy()
            self._basic_prof_widget = None
        if self._basic_skin_widget:
            self._basic_skin_widget.destroy()
            self._basic_skin_widget = None
        super(MechaBasicInfoWidget, self).destroy()
        return

    def hide(self):
        if not global_data.video_player.is_in_init_state():
            global_data.video_player.stop_video()
        super(MechaBasicInfoWidget, self).hide()
    ...
    def on_switch_to_mecha_type(self, mecha_type):
        ...
        if not self._basic_prof_widget:
            self._basic_prof_widget = MechaBasicInfoProficiencyWidget(self.parent, self.panel, mecha_type)
        else:
            self._basic_prof_widget.on_switch_mecha_type(mecha_type)
        if not self._basic_skin_widget:
            self._basic_skin_widget = MechaBasicSkinWidget(self.parent, self.panel, mecha_type)
        else:
