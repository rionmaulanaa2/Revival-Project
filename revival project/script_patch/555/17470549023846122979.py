...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/vscene/parts/gamemode/CMagicSurMode.py
...
class CMagicSurMode(CNormalBase):
    ...
    def init_mgr(self):
        super(CMagicSurMode, self).init_mgr()
        from logic.comsys.battle.Magic.MagicSurvivalBattleMgr import MagicSurvivalBattleMgr
        MagicSurvivalBattleMgr.finalize()
        MagicSurvivalBattleMgr()
    ...
    def create_mode_specified_ui(self):
        ...
        if global_data.game_mode.is_neutral_shop_env() and not self.is_in_spectate():
            if global_data.is_pc_mode:
                ...
                BattleAceCoinUI()
        global_data.ui_mgr.show_ui('MagicRuneConfUI', 'logic.comsys.battle.Magic')

    def destroy_ui(self):
        global_data.ui_mgr.close_ui('BattleAceCoinUI')
        global_data.ui_mgr.close_ui('TrainProgUI')
        global_data.ui_mgr.close_ui('MagicSurvivalGuideUI')
        global_data.ui_mgr.close_ui('MagicRuneConfUI')

    def on_finalize(self):
        super(CMagicSurMode, self).on_finalize()
        self.process_event(False)
        self.destroy_ui()
        global_data.magic_sur_battle_mgr.finalize()
