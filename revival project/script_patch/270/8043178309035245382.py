...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/observe_ui/ZombieFFAObserveEnd.py
...
class ZombieFFAObserveEnd(BasePanel):
    ...
    def on_init_panel(self, *args, **kwargs):
        super(ZombieFFAObserveEnd, self).on_init_panel(*args, **kwargs)
    ...
    def begin_show(self, settle_dict):
        ...
        group_mecha_dict = dict(settle_dict.get('group_mechas', []))
        self_group_id = global_data.player.logic.ev_g_group_id()
        self_group_mecha = group_mecha_dict.get(self_group_id, None)
        winner_mecha_id = settle_dict.get('winner_mecha_ids', [])
        battle_result = 'win'
        if self_group_mecha not in winner_mecha_id:
            battle_result = settle_dict.get('is_draw', True) or 'defeated' if 1 else 'draw'
        ...
        self.show_win_info(settle_dict)
        self.panel.PlayAnimation('defeat_score')
        return

    def show_win_info(self, settle_dict):
        if settle_dict.get('is_draw', False):
            self.panel.nd_win_info.setVisible(False)
            return
        winner_mecha = settle_dict.get('winner_mecha_ids', [])
        if not winner_mecha:
            self.panel.nd_win_info.setVisible(False)
            return
        ...
        bg_width, bg_height = nd_bg.GetContentSize()
        nd_info.setPosition(bg_position)
        nd_info.SetHorizontalAlign(1)
        nd_info.formatText()
        line_width = nd_info.getLineWidths()[0]
        nd_bg.SetContentSize(line_width + 90, bg_height)
        nd_bg.img_bg.SetContentSize(line_width + 90, bg_height)
