...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityChristmas/ActivityChristmasGun.py
...
import cc
PIC0 = 'gui/ui_res_2/activity/activity_202112/christmas_gun/img_christmas_light.png'

class ActivityChristmasGun(ActivityCollect):

    def __init__(self, dlg, activity_type):
        super(ActivityChristmasGun, self).__init__(dlg, activity_type)
    ...
    def on_init_panel(self):
        ...
        if btn_describe:
            ...
            def OnClick(btn, touch):
                ...
                dlg.set_show_rule(get_text_by_id(607171), get_text_by_id(conf.get('cRuleTextID', '')))
    ...
    def _init_gun_progress(self):
        christmas_num = global_data.player.get_item_num_by_no(70400018)
        player = global_data.player
        ...
        for idx, ui_item in enumerate(self.panel.list_item.GetAllItem()):
            ...
            if is_received:
                ...
                ui_item.btn_choose.SetFrames('', ['', PIC0, PIC0], False, None)
            elif can_receive:
                ui_item.btn_choose.SetFrames('', [PIC0, PIC0, PIC0], False, None)
            else:
                ui_item.btn_choose.SetFrames('', ['', PIC0, PIC0], False, None)
        ...
        self.panel.lab_progress.SetString(''.join((get_text_by_id(610300), str(christmas_num))))
        christmas_num = min(christmas_num, 100)
        self.panel.prog_inside.setPercentage(christmas_num)

        @self.panel.btn_gun_see.unique_callback()
        def OnClick(btn, touch):
            jump_to_display_detail_by_item_no('208105520')

        first_reward_is_received = player.has_receive_prog_reward(self._collect_task, 0)
        ...
        if last_reward_is_received:
            ...
            if task_left_time <= 86400:
                action_list = [cc.CallFunc.create(lambda : self.panel.bar_tips_riko.setVisible(True)),
    ...
    def refresh_time(self, parent_task):
        ...
        if task_left_time > 0:
            if task_left_time > ONE_HOUR_SECONS:
                self.panel.lab_tips_time.SetString(get_text_by_id(610171).format(get_readable_time_day_hour_minitue(task_left_time)))
            else:
                self.panel.lab_tips_time.SetString(get_text_by_id(610171).format(get_readable_time(task_left_time)))
        else:
            close_left_time = 0
