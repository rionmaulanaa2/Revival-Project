...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/battle/Granbelm/GranhackRuneConfUI.py
...
class GranhackRuneConfUI(BasePanel):
    PANEL_CONFIG_NAME = 'battle_hack/fight_moon_choose_button'
    ...
    def on_init_panel(self, *args, **kwargs):
        super(GranhackRuneConfUI, self).on_init_panel()
        self.init_params()
        self.init_widget()
    ...
    def init_widget(self):
        self.finish_tips_widget = global_data.uisystem.load_template_create('battle_tips/full_hack/i_gear_full_tips', self.panel)
        self.finish_tips_widget.setVisible(False)
        self.region_tips_widget = global_data.uisystem.load_template_create('battle_tips/full_hack/gear_up_tips', self.panel)
    ...
    def on_granbelm_rune_finish(self):
        if not self._rune_id:
            if self._rune_count == GRANBELM_MAX_RUNE_COUNT:
                self.panel.img_gear.SetDisplayFrameByPath('', granhack_utils.get_rune_pic_path(True))
    ...
    def _on_click_conf(self, *args):
        if not global_data.cam_lplayer or not global_data.cam_lplayer.ev_g_is_avatar():
            return
        if not self._rune_id:
            global_data.ui_mgr.show_ui('GranhackRuneListUI', 'logic.comsys.battle.Granbelm')
        else:
            desc_text = granhack_utils.get_rune_ability_dict(self._rune_id).get('desc')
    ...
    def _set_rune_display(self):
        ...
        self.panel.img_stone.SetDisplayFrameByPath('', granhack_utils.get_rune_ability_dict(self._rune_id).get('path'))
    ...
    def on_finalize_panel(self):
        ...
        super(GranhackRuneConfUI, self).on_finalize_panel()
        return

    def keyboard_toggle_moon_rune_list(self, msg, keycode):
        if global_data.ui_mgr.get_ui('GranhackRuneListUI'):
            global_data.ui_mgr.close_ui('GranhackRuneListUI')
        else:
            self._on_click_conf()