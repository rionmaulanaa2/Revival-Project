...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/lobby/EntryWidget/LobbyKizunaAIEntryWidget.py
...
class LobbyKizunaAIEntryWidget(LobbyEntryWidgetBase):

    def on_init_widget(self):
        super(LobbyKizunaAIEntryWidget, self).on_init_widget()
        self.panel.btn.BindMethod('OnClick', self.on_click_btn)
        self.panel.PlayAnimation('loop')

    def on_click_btn(self, *args):
        jump_to_ui_utils.jump_to_kizuna_ai_activity()
    ...
    def need_show_red_point(self):
        ...
        if not check_lobby_red_point():
            return False
        activity_list = activity_utils.get_ordered_activity_list(activity_const.WIDGET_KIZUNA_AI)
