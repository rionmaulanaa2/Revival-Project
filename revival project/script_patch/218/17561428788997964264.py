...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/gcommon/component/client/com_ai_ctrl/ComRemoteControl8005.py
...
class ComRemoteControl8005(UnitCom):
    ...
    def __init__(self):
        super(ComRemoteControl8005, self).__init__()

    def init_from_dict(self, unit_obj, bdict):
        super(ComRemoteControl8005, self).init_from_dict(unit_obj, bdict)
        self._rocket_jump_pos = None
        return

    def destroy(self):
        super(ComRemoteControl8005, self).destroy()
    ...
    def on_enter_state(self, sid):
        if sid == mecha_status_config.MC_SECOND_WEAPON_ATTACK:
            ...
            global_data.game_mgr.delay_exec(random.uniform(0.2, 1.2), cast_skill)
    ...
    def main_weapon_attack_start(self, target_id, pos, is_hit):
        if self.ev_g_shape_shift():
            ...
            global_data.game_mgr.delay_exec(random.uniform(0.5, 1.2), cast_skill)
    ...
    def get_dash_pos(self):
        ...
        if cur_yaw is None:
            ...
            if direction is None:
                return
            agl = random.uniform(-30.0, 30.0)
            agl = math.radians(agl)
            direction = direction * matrix.make_rotation_y(agl)
            r = random.uniform(20.0, 50.0) * NEOX_UNIT_SCALE
    ...
    def dash_start(self, dash_dir):
        if self._rocket_jump_pos:
            ...
            global_data.game_mgr.delay_exec(0.1, self.exec_dash)
            return
    ...
    def ext_start(self, pos, shape=True):
        if shape:
            if self.ev_g_shape_shift():
                self.send_event('E_SYNC_STATE_DATA', 'ext_suc', 2)
                return
            if self.ev_g_trans_status(mecha_status_config.MC_TRANSFORM):
                self.send_event('E_CTRL_FACE_TO', pos)
                self.ev_g_try_enter(mecha_status_config.MC_TRANSFORM)
                self.send_event('E_SYNC_STATE_DATA', 'ext_suc', 2)
            else:
                self.send_event('E_SYNC_STATE_DATA', 'ext_suc', 1)
        else:
            if not self.ev_g_shape_shift():
                self.send_event('E_SYNC_STATE_DATA', 'ext_suc', 2)
                return
            if self.ev_g_trans_status(mecha_status_config.MC_TRANSFORM):
                self.send_event('E_CTRL_FACE_TO', pos)
