...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/entities/RandomDeathBattle.py
...
import math3d
ITEM_TO_COLOR = {9924: '0x0054e0ff',9926: '0x0054e0ff',
   9930: '0x007f24ff',
   9931: '0x007f24ff',
   9932: '0x007f24ff',
   9933: '0x007f24ff',
...
class RandomDeathBattle(Battle):

    def init_from_dict(self, bdict, is_change_weapon=True):
        self.settle_timestamp = 0
        weapon_dict = bdict.get('weapon_dict', {})
        if 10 in weapon_dict:
            del weapon_dict[10]
        super(RandomDeathBattle, self).init_from_dict(bdict)
        self.area_id = bdict.get('area_id')
        global_data.death_battle_data.set_area_id(str(self.area_id))
        ...
        global_data.death_battle_data.set_mecha_list(bdict.get('mecha_list', []))
        spawn_rebirth_dict = bdict.get('spawn_rebirth_dict', {})
        global_data.death_battle_data.update_spawn_rebirth_data(spawn_rebirth_dict)
    ...
    def client_loading_end(self):
        self.call_soul_method('client_loading_end', ())

    def set_combat_weapons(self, weapon_dict):
        pass

    @rpc_method(CLIENT_STUB, (Int('itemid'),))
    def get_item_tip(self, itemid):
        item_text = '<color={}>{}</color>'.format(ITEM_TO_COLOR.get(itemid, '0x0054e0ff'), item_utils.get_item_name(itemid))
        msg = {'i_type': battle_const.RANDOM_DEATH_GET_ITEM_TIP,
           'icon_path': item_utils.get_item_pic_by_item_no(itemid),
           'content_txt': get_text_by_id(17260, (item_text,))
           }
        if global_data.cam_lplayer:
            global_data.cam_lplayer.send_event('E_SHOW_MAIN_BATTLE_MESSAGE', msg, battle_const.MAIN_NODE_COMMON_INFO)

    @rpc_method(CLIENT_STUB, (Int('itemid'),))
    def lost_item_tip(self, itemid):
        item_text = '<color={}>{}</color>'.format(ITEM_TO_COLOR.get(itemid, '0x0054e0ff'), item_utils.get_item_name(itemid))
        msg = {'i_type': battle_const.RANDOM_DEATH_LOST_ITEM_TIP,
           'content_txt': get_text_by_id(17261, (item_text, '<color=0xff0000ff>10</color>'))
           }
        if global_data.cam_lplayer:
            global_data.cam_lplayer.send_event('E_SHOW_MAIN_BATTLE_MESSAGE', msg, battle_const.MAIN_NODE_COMMON_INFO)

    @rpc_method(CLIENT_STUB, (Int('item_lost_time'),))
    def lost_item_time(self, item_lost_time):
        global_data.death_battle_data.set_item_lost_time(item_lost_time)

    @rpc_method(CLIENT_STUB, ())
    def get_weapon_tip(self):
        msg = {'i_type': battle_const.RANDOM_DEATH_GET_WEAPON_TIP
           }
        if global_data.cam_lplayer:
            global_data.cam_lplayer.send_event('E_SHOW_MAIN_BATTLE_MESSAGE', msg, battle_const.MAIN_NODE_COMMON_INFO)
    ...
    @rpc_method(CLIENT_STUB, (List('mecha_list'),))
    def set_mecha_list(self, mecha_list):
        global_data.death_battle_data.set_mecha_list(mecha_list)
