...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/battle/Settle/DuelEndSceneUI.py
...
class DuelEndSceneUI(BasePanel):
    PANEL_CONFIG_NAME = 'end/end_statistics_duel'
    ...
    def on_init_panel(self, group_num, settle_dict, reward, teaminfo, achievement):
        self.cur_stage = DuelEndSceneUI.SHOW_ACHIEVE
        ...
        if self._is_ob_settle():
            ...
            for eid, info in six.iteritems(self.teammate_info):
                if ObjectId(eid) == self._get_self_eid():
                    show_teaminfo[eid] = info

        else:
            p_settle_dict = settle_dict
            show_teaminfo = teaminfo
        self.model_loader.set_parameters(p_settle_dict, show_teaminfo, 0.9, 1.1, False)
        self.screen_capture_helper = ScreenFrameHelper()
    ...
    def init_settle_data(self):
        ...
        if 'settle_detail' not in self.settle_dict:
            self.settle_dict.update({'settle_detail': {my_group_id: {},enemy_group_id: {}}})
            log_error('can not find settle_detail', self.settle_dict)
        self._init_group_settle_data(self.settle_dict['settle_detail'][my_group_id], self.panel.temp_data.nd_blue.list_score, self.panel.temp_details.nd_blue.list_score, self.settle_dict['settle_detail'].get(enemy_group_id, {}))
        if enemy_group_id in self.settle_dict['settle_detail']:
            self._init_group_settle_data(self.settle_dict['settle_detail'][enemy_group_id], self.panel.temp_data.nd_red.list_score, self.panel.temp_details.nd_red.list_score, self.settle_dict['settle_detail'][my_group_id])
    ...
    def _init_group_settle_data(self, group_data, nd_abstract_list, nd_detail_list, other_group_data):
        ...
        for index, (eid, data) in enumerate(six.iteritems(group_data)):
            ...
            if self._is_ob_settle():
                ...
                abstract_item.btn_like.setVisible(False)
                abstract_item.btn_report.setVisible(False)
            abstract_item.list_mecha.SetInitCount(len(mecha_data))
            sum_damage = sum((int(damage) for _, damage, _ in mecha_data))
            all_enemy_damages = [ _data[7] for _index, (_eid, _data) in enumerate(six.iteritems(other_group_data)) ]
            sum_enemy_damage = 0
            enemy_damage = [ (0, 0, 0) for i in range(len(mecha_data)) ]
            if len(all_enemy_damages) > 0:
                enemy_damage = all_enemy_damages[0]
                sum_enemy_damage = sum((int(damage) for _, damage, _ in enemy_damage))
            sum_data_list = [sum_damage, sum_enemy_damage]
            for i, (mecha_id, damage, kill_num) in enumerate(mecha_data):
                damage = int(damage)
                nd_data_mecha_data = abstract_item.list_mecha.GetItem(i)
                nd_data_mecha_data.img_mecha.SetDisplayFrameByPath('', 'gui/ui_res_2/battle/role/mech_%s.png' % str(mecha_id))
                mecha_name_item = battle_id_to_mecha_lobby_id(mecha_id)
                nd_data_mecha_data.lab_name_mecha.SetString(item_utils.get_lobby_item_name(mecha_name_item))
                if global_data.game_mode.is_mode_type((game_mode_const.GAME_MODE_DUEL,)):
                    ...
                    if i < len(record_list):
                        nd_data_mecha_data.img_mecha.SetColor('#SC' if record_list[i] == ROUND_LOSE else '#SW')
                    else:
                        nd_data_mecha_data.img_mecha.SetColor('#SW')
                    data_list = [
                     damage, 0 if i >= len(enemy_damage) else int(enemy_damage[i][1])]
                    nd_data_mecha_data.list_prog.SetInitCount(len(data_list))
                    for j in range(len(data_list)):
                        nd_prog_data = nd_data_mecha_data.list_prog.GetItem(j)
                        nd_prog_data.lab_data.SetString(format_popular_num(int(data_list[j])))
                        if sum_data_list[j]:
                            percent = data_list[j] / float(sum_data_list[j]) * 100.0 if 1 else 0
                            nd_prog_data.lab_persent.SetString('%d%%' % percent)
                            nd_prog_data.progress.SetPercentage(percent)

                else:
                    nd_data_mecha_data.temp_mech_head.img_destoryed.setVisible(i < mecha_dead_count)
    ...
    def _init_achievement(self):
        ...
        if is_mvp and not self._is_ob_settle() and (self.win_ending or self.draw_ending):
            ...
            nd_mvp.img_times.setVisible(False)
            nd_mvp.bg_times.setVisible(False)
    ...
    def on_click_btn_next(self, *args):
        ...
        elif self.cur_stage == DuelEndSceneUI.SHOW_ALL_SCORE:
    ...
    def show_btn_again(self):
        if not global_data.battle:
            return
        battle_tid = global_data.battle.battle_tid
        bSupportCustom = confmgr.get('battle_config', str(battle_tid), default={}).get('bSupportCustom', 0)
        if bSupportCustom > 0:
            return
