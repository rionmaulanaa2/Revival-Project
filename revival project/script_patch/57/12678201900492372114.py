...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/clan/ClanLogList.py
...
class ClanLogList(ClanPageBase):

    def __init__(self, dlg):
        self.global_events = {'clan_members_request_list': self.init_widget,
           'message_on_players_detail_inf': self.init_log_list
           }
        super(ClanLogList, self).__init__(dlg)

    def on_init_panel(self):
        super(ClanLogList, self).on_init_panel()
        self._list_sview = None
        global_data.player.request_clan_request()
        return

    def on_finalize_panel(self):
        super(ClanLogList, self).on_finalize_panel()
        if self._list_sview:
    ...
    def refresh_panel(self):
        super(ClanLogList, self).refresh_panel()
    ...
    def request_player_detail_info(self, uid_list):
        ...
        if r_uid_list:
            global_data.player.request_players_simple_inf(r_uid_list)
        else:
            self.init_log_list(None)
        return
    ...
    def init_widget(self):
        ...
        print('>> ClanLogList', request_member_list)
    ...
    def on_init_list_item(self, item_widget, uid):
        ...
        player_info = global_data.message_data.get_player_detail_inf(uid)
        role_head_utils.init_role_head(item_widget.temp_head, player_info[HEAD_FRAME], player_info[HEAD_PHOTO])
        item_widget.lab_name.SetString(player_info[C_NAME])
        win_count = 10
        total_count = 100
        item_widget.lab_content_session.SetString(str(win_count))
        item_widget.lab_content_win_rate.SetString('{}%'.format(utilities.safe_percent(win_count, total_count)))
