...
class BpAdvisementUI(BasePanel):
    PANEL_CONFIG_NAME = 'battle_pass/open_bp_award'
    ...
    def on_init_panel(self):
        ...
        for idx, item_id in enumerate(open_reward):
            tag_node = getattr(self.panel.nd_content, 'tag_%s' % (idx + 1), None)
            if tag_node:
                tag_node.img_tag.tag.lab_name.SetString(get_lobby_item_name(item_id))
                tag_node.img_tag.bar_tag.temp_level.SetDisplayFrameByPath('', get_skin_rare_degree_icon(item_id))
