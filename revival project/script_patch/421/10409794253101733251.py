...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityAIConcert/ActivityVeteranRecall.py
from __future__ import absolute_import
import six_ex
from logic.comsys.activity.ActivityTemplate import ActivityBase
from logic.gutils.jump_to_ui_utils import jump_to_display_detail_by_item_no
from logic.gcommon.common_utils.local_text import get_text_by_id
from logic.gutils.item_utils import get_lobby_item_name, get_skin_rare_degree_icon, get_lobby_item_belong_name, get_lobby_item_pic_by_item_no
...
class ActivityVeteranRecall(ActivityBase):
    ...
    def init_parameters(self):
        ...
        self.point_item_no = self.ui_data['point_item_no']
        self.cur_points = global_data.player.get_item_num_by_no(self.point_item_no)
        self.ex_mall_item_list = self.ui_data['mall_item_list']
        self.ex_item_list = [ mall_utils.get_goods_item_no(str(mall_item)) for mall_item in self.ex_mall_item_list ]
        self.cb_point = self.ui_data['cb_point']
        self.cb_finish_days = self.ui_data.get('cb_finish_days', 0)
        self.act_end_time = self.activity_conf.get('cEndTime', 0)
    ...
    def init_ui_events(self):
        self.panel.lab_time.SetString(self.ui_data['time_text_id'])

        @self.panel.btn_question.unique_callback()
        def OnClick(_btn, _touch, *args):
            from logic.comsys.common_ui.GameRuleDescUI import GameRuleDescUI
            dlg = GameRuleDescUI()
            title, rule = self.activity_conf['cDescTextID'], self.activity_conf['cRuleTextID']
            dlg.set_show_rule(get_text_by_id(title), get_text_by_id(rule))

        self.nd_points.SetString(str(self.cur_points))
        last_reward_item = self.ex_item_list[-1]
        self.panel.nd_content.nd_exchange.nd_reward_01.lab_name.SetString(get_lobby_item_belong_name(last_reward_item) + ' ' + get_lobby_item_name(last_reward_item))
        tag_img = get_skin_rare_degree_icon(self.ex_item_list[-1], 1)
        self.panel.nd_content.nd_exchange.nd_reward_01.nd_kind.bar_level.SetDisplayFrameByPath('', tag_img)
        self.panel.nd_content.nd_exchange.nd_reward_01.img_reward.SetDisplayFrameByPath('', get_lobby_item_pic_by_item_no(last_reward_item))

        @self.panel.nd_content.nd_exchange.nd_reward_01.btn_detail.unique_callback()
        def OnClick(_btn, _touch, *args):
            jump_to_display_detail_by_item_no(self.ex_item_list[-1])
    ...
    def check_finish(self):
        ...
        if cur_time + self.cb_finish_days * time_utility.ONE_DAY_SECONDS > self.act_end_time:
            return True
        return False
