...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/items_book_ui/ProjectionKillGetUseBuyWidget.py
...
class ProjectionKillGetUseBuyWidget(object):
    ...
    def update_event(self):
        ...
        if self.cancel_btn:
            self.cancel_btn.btn_common_big.BindMethod('OnClick', self.on_click_cancel_btn)
        if self.jump_btn:
    ...
    def update_cancel_widget(self):
        if self.cancel_btn:
            self.cancel_btn.setVisible(int(self.item_no) == global_data.player.get_current_selected_projection_kill_no())
    ...
    def refresh_widget(self):
        ...
        can_use = item_can_use and int(self.item_no) != global_data.player.get_current_selected_projection_kill_no()
    ...
    def on_click_use_btn(self, *args):
        if not self.item_no:
            return
        global_data.player and global_data.player.try_set_selected_projection_kill_no(self.item_no)

    def on_click_jump_btn(self, *args):
        item_utils.jump_to_ui(self.item_no)

    def on_click_cancel_btn(self, *args):
        global_data.player and global_data.player.try_set_selected_projection_kill_no(0)
