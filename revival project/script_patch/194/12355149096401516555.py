...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityInviteLink.py
...
class ActivityInviteLink(WindowSmallBase):
    PANEL_CONFIG_NAME = 'activity/activity_new_domestic/i_activity_recruit_tips'
    DLG_ZORDER = NORMAL_LAYER_ZORDER_1
    TEMPLATE_NODE_NAME = 'temp_window'
    TITLE_TEXT_ID = 607803
    UI_ACTION_EVENT = {}
    GLOBAL_EVENT = {'message_on_players_detail_inf': 'on_players_detail_inf',
       'message_on_enlist_verify': 'on_enlist_verify'
       }
    ...
    def on_init_panel(self, *args, **kwargs):
        super(ActivityInviteLink, self).on_init_panel()
        ...
        enlist_from_uid = global_data.player.get_enlist_from_uid()
        if enlist_from_uid:
            invite_code = str(enlist_from_uid)
        ...
        reward_id = c_recruit_data.GetEnlistCommon().get('enlist_verify_gift', {}).get('value', 0)
        template_utils.init_common_reward_list(self.panel.list_items, reward_id)
        self.panel.lab_player.SetString('')
        if global_data.player.has_enlist_verify_gift():
            self.panel.nd_middle_01.setVisible(False)
            self.panel.nd_middle_02.setVisible(True)
            if self.invite_code:
                message_data = global_data.message_data
                uid = int(self.invite_code)
                if not G_IS_NA_USER:
        ...
        def OnClick(btn, touch):
            if not self._cur_show_uid:
                return
            global_data.player.try_enlist_from_code(str(self._cur_show_uid))
        ...
        def OnClick(btn, touch):
            if not self._cur_show_uid:
                return
            self.close()
            global_data.player.receive_enlist_verify_gift()
    ...
    def on_players_detail_inf(self, *argv):
        ...
        if not self.invite_code:
            ...
            online = True if state != const.STATE_OFFLINE else False
