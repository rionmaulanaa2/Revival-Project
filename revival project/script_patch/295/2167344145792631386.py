...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/items_book_ui/RoleItemSelectWidget.py
...
class RoleItemSelectWidget(object):
    ...
    def check_role_rp_dict(self, role_id):
        click_time = global_data.lobby_red_point_data.get_item_type_rp_click_time(self.item_type, role_id)
        ...
        for k in six_ex.keys(item_dict):
            ...
            if not item_info:
                continue
            if item_info.rp and item_info.get_create_time() > click_time:
                return True
    ...
    def init_role_data(self):
        ...
        for k, v in six.iteritems(self.role_config):
            if not role_utils.is_role_publish(k):
                continue
            ...
            if open_role_id_list and k not in open_role_id_list:
                continue
            if not role_utils.check_show_role_panel(k):
                continue
...
Parse error at or near `CALL_FUNCTION_2' instruction at offset 29
    ...
    def on_create_item(self, lst, index, item_widget):
        ...
        if index < len(self.role_list):
            ...
            item_widget.btn.SetText(mall_utils.get_goods_name(role_info[1]['goods_id']))
    ...
    def on_click_item(self, index, *args):
        if not self.valid:
            ...
            if item_widget:
                ...
                if role_id in self.role_rp_dict:
                    self.panel.SetTimeOut(0.03, lambda : global_data.lobby_red_point_data.set_item_type_rp_click_time(self.item_type, role_id))
            return
    ...
    def init_select_item(self, index, off_set=None):
        ...
        if hasattr(list_item, 'scroll_Load') and index > 0:
    ...
    def jump_to_item_no(self, item_no):
        if not item_no:
            return
        belong_no = items_book_utils.get_interaction_belong_to_role(item_no, self.item_type)
        if belong_no:
            try:
                index = self.role_id_list.index(str(belong_no))
            except:
                index = 0
