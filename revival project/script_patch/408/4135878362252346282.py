...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/gcommon/skill/client/SkillThrowable.py
...
class SkillThrowable(SkillCd):

    def __init__(self, skill_id, unit_obj, data):
        super(SkillThrowable, self).__init__(skill_id, unit_obj, data)
    ...
    def do_skill(self, stage, position, forward):
        ...
        position = position + forward * NEOX_UNIT_SCALE * 8
        ...
        if 1 <= stage <= 3 and model.has_socket(socket):
            ...
            up = mat.rotation.up
        target, target_id = self._get_aim_target_id()
        if target:
            ...
            if model:
                ...
                if matrix:
                    ...
                    forward = aim_pos - position
                    if not forward.is_zero:
                        forward.normalize()
        throw_item = {'item_itype': self._throw_item_no,'uniq_key': ExploderID.gen(global_data.battle_idx),
           'position': (
        ...
        if global_data.is_local_editor_mode:
            import time
            grenade_conf = confmgr.get('grenade_config', str(self._throw_item_no))
            throw_item['speed'] = grenade_conf['fSpeed']
            throw_item['mass'] = grenade_conf['fMass']
            throw_item['gravity'] = grenade_conf['fGravity']
            throw_item['begin_time'] = time.time()
            throw_item['last_time'] = grenade_conf['fTimeFly']
            throw_item['throw_speed_add_rate'] = 0
            throw_item['owner_id'] = self._unit_obj.id
            throw_item['trigger_id'] = self._unit_obj.id
            throw_item['faction_id'] = 1
            global_data.emgr.scene_add_throw_item_event.emit(throw_item)
        return (
         throw_item, stage)
