...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/mecha_ui/MechaTransUI.py
...
UNHEALTH_PROGRESS_PIC = [
 'gui/ui_res_2/battle/attack/shield_blood_prog.png',
 'gui/ui_res_2/battle/attack/shield_blood_prog_red.png']
SKILL_SPEED_UP = 850151
RECOVER_RATE = 0.05
from common.const import uiconst

class MechaTransUI(BasePanel):
    MECHA_STATE_PIC = 'gui/ui_res_2/battle/drive/info_mech.png'
    VEHICLE_STATE_PIC = 'gui/ui_res_2/battle/drive/info_vehical.png'
    PANEL_CONFIG_NAME = 'drive/drive_common_btn'
    DLG_ZORDER = BASE_LAYER_ZORDER
    UI_VKB_TYPE = uiconst.UI_VKB_NO_EFFECT
    HOT_KEY_CHECK_VISIBLE = True
    UI_ACTION_EVENT = {'get_off.OnClick': 'on_get_off',
       'switch.OnClick': 'on_switch_transform',
       'speed_up.OnClick': '_on_speed_up'
       }
    HOT_KEY_FUNC_MAP_SHOW = {'car_rush': {'node': 'speed_up.temp_pc'},'car_transform': {'node': 'switch.temp_pc'},'get_off_skateboard_or_vehicle': {'node': ['get_off.temp_pc']}}
    ASSOCIATE_UI_LIST = [
     'FireRockerUI', 'PostureControlUI', 'FightLeftShotUI', 'BulletReloadUI']

    def on_init_panel(self):
        self.init_parameters()
        self.set_associated_ui_visible(MechaTransUI.ASSOCIATE_UI_LIST, False)
        ...
        if global_data.is_pc_mode:
            ...
            self.panel.speed_up.setVisible(True)
    ...
    def _set_mecha(self, mecha):
        ...
        if mecha and mecha.logic:
            ...
            self.handle_ui_by_mecha_pattern(pattern)
    ...
    def handle_ui_by_mecha_pattern(self, pattern, *args, **argv):
        if mconst.MECHA_TYPE_VEHICLE == pattern:
            self.panel.speed_up.setVisible(True and not global_data.is_pc_mode)
        elif mconst.MECHA_TYPE_NORMAL == pattern:
            self.panel.speed_up.setVisible(False and not global_data.is_pc_mode)
            self.panel.lab_speed.SetString('--')

    def on_finalize_panel(self):
        self._set_mecha(None)
        self.on_player_setted(None)
        self.set_associated_ui_visible(MechaTransUI.ASSOCIATE_UI_LIST, True)
    ...
    def process_bind_mecha_event(self, lmecha, is_bind=True):
        if lmecha and lmecha.is_valid():
            ...
            ope_func('E_PATTERN_HANDLE', self.handle_ui_by_mecha_pattern)
    ...
    def on_switch_transform(self, *args):
        from logic.gutils.move_utils import can_move
        if not can_move():
            global_data.game_mgr.show_tip(2353)
            return
        if self._cur_mecha and self._cur_mecha.logic:
            self._cur_mecha.logic.send_event('E_TRY_TRANSFORM')
    ...
    def _on_speed_up(self, *args):
        ...
        if not can_move():
            return
        if self._cur_mecha and self._cur_mecha.logic:
            self._cur_mecha.logic.send_event('E_TRY_VEHICLE_DASH')
    ...
    def set_btn_transform_visible(self, visible):
        self.panel.switch.setVisible(visible)

    def set_btn_speed_visible(self, visible):
        self.panel.speed_up.setVisible(visible)
