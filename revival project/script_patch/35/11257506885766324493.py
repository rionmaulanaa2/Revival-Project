...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/gcommon/skill/client/SkillSlash8021.py
...
class SkillSlash8021(SkillCd):

    def __init__(self, skill_id, unit_obj, data):
        super(SkillSlash8021, self).__init__(skill_id, unit_obj, data)
        ...
        self._throw_item_no = 802152
        return

    def add_throwable(self, stage, position, forward):
        ...
        socket = 'fx_daoguang0{}'.format(stage + 3)
        if stage in (0, 1) and model.has_socket(socket):
            mat = model.get_socket_matrix(socket, world.SPACE_TYPE_WORLD)
            up = mat.rotation.up
        width_up_factor = self._unit_obj.ev_g_add_attr('8021_secwp_width_up_factor', 0)
        throw_item = {'uniq_key': ExploderID.gen(global_data.battle_idx),
           ...
           'up': (
                up.x, up.y, up.z),
           'use_rot_mat': 1,
           'col_width': info.get('energy_width', 6) * (1 + width_up_factor)
           }
