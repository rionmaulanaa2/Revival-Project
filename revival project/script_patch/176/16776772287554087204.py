...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityFairyland/ActivityFairylandDaily.py
...
from logic.gcommon.common_const.activity_const import ACTIVITY_FAIRYLAND_FOOD_EXCHANGE, ACTIVITY_FAIRYLAND_FOOD_DAILY_TASK, ACTIVITY_FAIRYLAND_FOOD_EXCHANGE_STEAM
from logic.gcommon.item.item_const import ITEM_UNRECEIVED
import cc
EXCHANGE_COIN_ITEM_NO = 50600030
ITEM_ID_LST = [(201800671, 694000012), (208200104, 694000013), (30710016, 694000014), (50101101, 694000016), (50920102, 694000017)]
ITEM_ID_LST_NA = [(50101223, 694000012), (50920103, 694000013), (50500031, 694000014), (50101101, 694000015), (50920102, 694000016)]
ITEM_ID_LST_STEAM = [(50101223, 694000021), (50920103, 694000022), (50500031, 694000023), (50101101, 694000024), (50920102, 694000025)]

class ActivityFairylandDaily(ActivityCollect):

    def __init__(self, dlg, activity_type):
        super(ActivityFairylandDaily, self).__init__(dlg, activity_type)
        ...
        self.panel.pnl_icon.setVisible(False)
        return

    def on_finalize_panel(self):
        super(ActivityFairylandDaily, self).on_finalize_panel()
        self.sub_widget = None
        return
    ...
    def init_items(self):
        ...
        if G_IS_NA_PROJECT:
            item_id_list = ITEM_ID_LST_NA
        elif global_data.channel.is_steam_channel():
            item_id_list = ITEM_ID_LST_STEAM
        for idx in range(len(item_id_list)):
            ...
            img_item_node = getattr(self.panel, 'img_item_%s' % (idx + 1))
            if idx == 0:
                if G_IS_NA_PROJECT or global_data.channel.is_steam_channel():
                    item_path = 'gui/ui_res_2/activity/activity_202108/subway/pnl_img_item_1.png'
                else:
                    item_path = 'gui/ui_res_2/activity/activity_202108/subway/pnl_subway_right_mecha.png'
            elif idx == 1 and (G_IS_NA_PROJECT or global_data.channel.is_steam_channel()):
                item_path = 'gui/ui_res_2/activity/activity_202108/subway/pnl_img_item_2.png'
            else:
            ...
            def OnClick(btn, touch, item_id=item_id):
                if global_data.channel.is_steam_channel():
                    jump_to_activity(ACTIVITY_FAIRYLAND_FOOD_EXCHANGE_STEAM)
                else:
                    jump_to_activity(ACTIVITY_FAIRYLAND_FOOD_EXCHANGE)
    ...
    def on_init_panel(self):
        ...
        if G_IS_NA_PROJECT or global_data.channel.is_steam_channel():
            self.panel.img_titile.setVisible(False)
            self.panel.img_title_na.setVisible(True)
            text_id = 907104
        else:
            self.panel.img_titile.setVisible(True)
            self.panel.img_title_na.setVisible(False)
            text_id = 907103
        self.panel.lab_tdescribe.setString(get_text_by_id(text_id))
        self.show_list()
    ...
    def init_parameters(self):
        super(ActivityFairylandDaily, self).init_parameters()
        self.task_id = confmgr.get('c_activity_config', self._activity_type, 'cTask', default='')

    def refresh_time(self, parent_task):
        task_id = confmgr.get('c_activity_config', ACTIVITY_FAIRYLAND_FOOD_DAILY_TASK, 'cTask', default='')
        exchange_id = confmgr.get('c_activity_config', ACTIVITY_FAIRYLAND_FOOD_EXCHANGE, 'cTask', default='')
    ...
    def set_show(self, show, is_init=False):
        super(ActivityFairylandDaily, self).set_show(show, is_init)
        ...
        if show:
            self.panel.runAction(cc.Sequence.create([
             ...
             cc.DelayTime.create(self.panel.GetAnimationMaxRunTime(show_animation_name)),
