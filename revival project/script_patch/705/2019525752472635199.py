...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/Activity202111/ActivityFlamingoSevenDayLogin.py
...
class ActivityFlamingoSevenDayLogin(ActivityCollect):

    def init_parameters(self):
        super(ActivityFlamingoSevenDayLogin, self).init_parameters()
    ...
    def show_list(self):
        ...
        coupon_dict = {1440409: [610209, 50101165],1440411: [610208, 50101166],1440414: [610207, 50101167]}
        from logic.gutils.mall_utils import item_has_owned_by_item_no
        self.panel.lab_get_coupon.SetString(610101)
        show_item_id = 50101167
        for task_id in sorted(six_ex.keys(coupon_dict), reverse=True):
            if ITEM_RECEIVED == global_data.player.get_task_reward_status(task_id):
                self.panel.lab_get_coupon.SetString(coupon_dict[task_id][0])
                self.panel.lab_get_coupon.SetColor(16776566)
                show_item_id = coupon_dict[task_id][1]
                break

        if ITEM_RECEIVED == global_data.player.get_task_reward_status(1440414):
