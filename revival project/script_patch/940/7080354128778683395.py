...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/role_profile/WardrobeWidget.py
...
class WardrobeWidget(BaseUIWidget):
    ...
    def __init__(self, parent, panel):
        self.global_events = {'bond_update_role_level': self.on_bond_udpate,
           ...
           'bond_role_gift': self.show_skills
           }
        super(WardrobeWidget, self).__init__(parent, panel)
        self.role_id = 0
        self.chosen_item = 0
        self.preview_item = 0
        self._ui_role_id = 0
        self.role_skin_config = confmgr.get('role_info', 'RoleSkin', 'Content')
        self.list_container = ScaleableHorzContainer(self.panel.list_skin, self.panel.nd_cut, self.panel.list_skin_dot, self._skin_move_select_callback, self._skin_up_select_callback, self._skin_select_begin_callback)
        ...
        self.panel.btn_select.BindMethod('OnClick', self.show_filter)
        ...
        self.guide_xingyuan_btn_set()
        self.force_skin_list = []
        self.force_group_skin = None
    ...
    def on_buy_good_success(self, item_no):
        ...
        self.update_battlepass_free_trial_template()
    ...
    def show_panel(self, flag):
        self.panel and self.panel.setVisible(flag)
    ...
    def destroy(self):
        ...
        super(WardrobeWidget, self).destroy()
        return
    ...
    def set_force_skin_list(self, skin_list, skin_id):
        self.force_skin_list = skin_list
        self.preview_item = dress_utils.get_top_skin_id_by_skin_id(skin_id) or skin_id
        self.force_group_skin = skin_id
        belong_no = item_utils.get_lobby_item_belong_no(skin_id)
        self.role_id = belong_no
    ...
    def use_default_skin(self):
        ...
        if chosen_item in self._show_skin_list:
            ...
            top_skin_id = dress_utils.get_top_skin_id_by_skin_id(chosen_item) or chosen_item
            if top_skin_id in self._show_top_skin_list:
                self._selected_index = self._show_top_skin_list.index(chosen_item)
                chosen_item = self._show_skin_list[self._selected_index]
                self.fix_group_clothing_selected_index(chosen_item)
            else:
                self._selected_index = 0
                self.group_clothing_selected_index = 0
                log_error('use_default_skin:failed to find chosen item in show skin list', chosen_item, self._show_skin_list)

    def use_selected_skin(self):
        chosen_item = self.preview_item
        self.group_clothing_selected_index = 0
        if chosen_item in self._show_skin_list:
            self._selected_index = self._show_skin_list.index(chosen_item)
            self.fix_group_clothing_selected_index(chosen_item, force_group_skin_id=self.force_group_skin)
        else:
            top_skin_id = dress_utils.get_top_skin_id_by_skin_id(chosen_item) or chosen_item
            if top_skin_id in self._show_top_skin_list:
                self._selected_index = self._show_top_skin_list.index(chosen_item)
                ...
                self._selected_index = 0
                self.group_clothing_selected_index = 0
                log_error('use_selected_skin:failed to find chosen item in show skin list', chosen_item, self._show_skin_list)
    ...
    def refresh_all_content(self):
        if not self.force_skin_list:
            ...
            self._ui_role_id = self.role_id
            self._skin_list = self.force_skin_list
            self._top_skin_list = [ dress_utils.get_top_skin_id_by_skin_id(i) for i in self._skin_list ]
            self._cur_level = LEVEL_ALL
            self.refresh_own_count()
            self.update_clothing_by_role(LEVEL_ALL)
            self.update_bond_level()
            self.use_selected_skin()
        self.panel.choose_list.setVisible(False)
        if self.force_skin_list:
            self._skin_move_select_callback(self._selected_index, self.force_group_skin)
        else:
    ...
    def update_battlepass_free_trial_template(self):
        battle_pass_utils.update_battlepass_free_trial_template(self.panel.temp_bp_free, self._ui_role_id)
    ...
    def get_player_skin_list(self):
        if not self.force_skin_list:
            ...
            top_skin_list = []
            for item_id in self.force_skin_list:
                if item_utils.can_open_show(item_id):
                    top_skin_list.append(dress_utils.get_top_skin_id_by_skin_id(item_id))
    ...
    def update_clothing_by_role(self, level):
        ...
        if not self.force_skin_list:
            self._show_skin_list = self._level_skin_list[level] if 1 else self.force_skin_list
            self._show_top_skin_list = self.force_skin_list or self._level_top_skin_list[level]
        else:
            self._show_top_skin_list = [ dress_utils.get_top_skin_id_by_skin_id(i) for i in self._show_skin_list ]
        self._skin_id_to_item = {}
    ...
    def _skin_move_select_callback(self, selected_index, force_group_skin=None):
        self._skin_up_select_callback(selected_index, force_group_skin)
    ...
    def _skin_select_begin_callback(self):
        if not global_data.video_player.is_in_init_state():
            global_data.video_player.stop_video()
    ...
    def buy_skin(self, is_default_skin, clothing_id, top_clothing_id, skin_data, goods_id):
        ...
        if not is_owned:
            ...
            global_data.player.install_role_skin_scheme(self.role_id, top_clothing_id, clothing_id, {FASHION_POS_SUIT: clothing_id})
    ...
    def refresh_own_count(self):
        has_func = global_data.player.has_item_by_no if global_data.player else (lambda x: 0)
        self._level_skin_list = {}
    ...
    def show_filter(self, *args):
        template_utils.init_common_choose_list(self.panel.choose_list, self._level_skin_count, self.on_filter_skin, close_cb=self.on_filter_panel_close)
        self.panel.choose_list.setVisible(True)
        self.panel.img_arrow.setRotation(180)

    def on_filter_panel_close(self):
        self.panel.img_arrow.setRotation(0)

    def on_filter_skin(self, index):
        level = self._level_skin_count[index]['rare_degree']
        if level == self._cur_level:
            return
        self._cur_level = level
        self.update_clothing_by_role(level)
        skin_id = self._show_skin_list[0]
        self.jump_to_skin(skin_id)
        self.update_showed_skin_count()

    def update_showed_skin_count(self):
        for info in self._level_skin_count:
            if info['rare_degree'] == self._cur_level:
                show_name = info['name'][0] + ' ' + info['name'][1]
                self.panel.lab_title.SetString(show_name)
    ...
    def refresh_bond_redpoint(self):
        ...
        nd_role_level.btn_details.img_tips.setVisible(can_get)
    ...
    def guide_xingyuan_btn_set(self):
        if global_data.lobby_guide_mgr:
            if global_data.lobby_guide_mgr.check_can_activate_by_guide_name('xingyuan_guide_roleinfo'):
                global_data.lobby_guide_mgr.switch_node_guide(self.panel.btn_set, 'xingyuan_guide_roleinfo', False)

    def on_deactivate_guide_by_name(self, guide_name):
        if guide_name != 'xingyuan_guide_roleinfo':
            return
        if global_data.lobby_guide_mgr:
            if not global_data.lobby_guide_mgr.check_can_activate_by_guide_name('xingyuan_guide_roleinfo'):
                global_data.lobby_guide_mgr.switch_node_guide(self.panel.btn_set, 'xingyuan_guide_roleinfo', True)

    def on_click_fold(self, *args):
        global_data.emgr.fold_mecha_details_widget.emit()

    def show_skills(self, *args):
        template_utils.init_bond_skills(self.panel.nd_role_level, self.role_id, self.parent.panel.list_price)
