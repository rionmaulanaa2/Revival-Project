...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/Activity202403/ActivityCommonTask.py
...
class ActivityCommonTask(ActivityCollect):

    def __init__(self, dlg, activity_type):
        super(ActivityCommonTask, self).__init__(dlg, activity_type)
    ...
    def show_list(self):
        ...
        for i, task_id in enumerate(self.task_list):
            ...
            if not nd:
                continue
            if i > 0:
                progress_nd = getattr(self.panel, 'prog_%d' % i)
                if progress_nd:
                    prog = task_utils.get_total_prog(task_id)
                    last_prog = task_utils.get_total_prog(self.task_list[i - 1])
                    total_delta_prog = prog - last_prog
                    cur_delta_prog = player.get_task_prog(task_id) - last_prog
                    if total_delta_prog > 0 and cur_delta_prog > 0:
                        progress_nd.SetPercentage(float(cur_delta_prog) / total_delta_prog * 100)
                    else:
                        progress_nd.SetPercentage(0)
            ...
            if reward_id:
                ...
                nd.bar_tips.setVisible(status == ITEM_UNGAIN)
                if status == ITEM_RECEIVED:
                    ...
                    nd.lab_task.SetColor(13158600)
                    nd.lab_day.SetColor(13158600)
                else:
                ...
                if status == ITEM_UNRECEIVED:
                    ...
                    nd.lab_task.SetColor('#SW')
                    nd.lab_day.SetColor(16751185)
                else:
