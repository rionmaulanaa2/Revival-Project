...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/rank/AssaultRankWidget.py
...
TYPE_TO_TEXT = {rank_const.RANK_TYPE_ASSAULT_DPM: 18365,
   rank_const.RANK_TYPE_ASSAULT_KPM: 18363
   }

class AssaultRankWidget(BaseRankWidget):

    def __init__(self, parent_panel, nd, template_pos, rank_info):
        super(AssaultRankWidget, self).__init__(rank_info)
    ...
    def refresh_show_text(self):
        self._template_root.lab_score.SetString(TYPE_TO_TEXT[self.cur_rank_type])
    ...
    def refresh_item(self, panel, data):
        ...
        if data[2]:
            if self.cur_rank_type == rank_const.RANK_TYPE_ASSAULT_DPM:
                panel.lab_number.SetString(str(int(data[2])))
            else:
                panel.lab_number.SetString('%.2f' % data[2])
        else:
