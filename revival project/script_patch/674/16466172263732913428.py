...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/battle/pve/PVEBoxMarkWidget.py
...
from logic.gcommon.common_const.pve_const import PVE_BOX_TYPE_ENERGY, PVE_BOX_TYPE_BREAK, PVE_BOX_STATE_CLOSE, PVE_BOX_STATE_OPEN
from logic.gcommon.common_utils.local_text import get_text_by_id

class PVEBoxMarkWidget(object):
    MAX_COUNT = 5
    DEFAULT_OFFSET = 8.0
    TEMPLATE = {PVE_BOX_TYPE_ENERGY: 'battle_tips/pve/i_pve_energy_mark',
       PVE_BOX_TYPE_BREAK: 'battle_tips/pve/i_pve_breakthrough_mark'
       }
    ...
    def process_events(self, is_bind):
        econf = {'pve_box_close_event': self.on_cancel_box,
           'pve_box_open_event': self.on_track_box,
           'pve_box_opened_event': self.on_cancel_box,
           'pve_get_box_tracked_event': self.on_get_track_event
           }
    ...
    def on_track_box(self, unit, show_tip=False):
        ...
        if eid in self.unit_dict:
            if eid in self.widget_dict:
                self.check_widget_state(unit)
            return
        self.unit_dict[eid] = unit
        self.type_dict[eid] = unit.ev_g_pve_box_type()
        self.adjust_mark()
    ...
    def init_mark(self, unit):
        eid = unit.id
        b_type = self.type_dict[eid]
        self.widget_dict[eid] = self.init_widget(b_type)
        self.check_widget_state(unit)
    ...
    def init_widget(self, b_type):
        widget = global_data.uisystem.load_template_create(self.TEMPLATE[b_type], self.panel)
