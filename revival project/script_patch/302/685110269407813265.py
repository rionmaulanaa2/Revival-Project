...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/gcommon/behavior/PVEMonsterBrambleLogic.py
...
class MonsterBrambleBase(MonsterStateBase):
    ...
    def init_from_dict(self, unit_obj, bdict, sid, info):
        super(MonsterBrambleBase, self).init_from_dict(unit_obj, bdict, sid, info)
        self.init_params()
    ...
    def on_init_complete(self):
        super(MonsterBrambleBase, self).on_init_complete()
        self.register_bramble_callbacks()
        self.reset_hit_range()

    def register_bramble_callbacks(self):
    ...
    def reset_hit_range(self):
        ...
        self.slash_checker.update_max_check_range(self.max_hit_dis)
    ...
    def enter(self, *args):
        super(MonsterBrambleBase, self).enter(*args)
    ...
    def update(self, dt):
        super(MonsterBrambleBase, self).update(dt)
    ...
    def exit(self, *args):
        super(MonsterBrambleBase, self).exit(*args)
        self.slash_checker.end_check()
        if self.aim_turn:
    ...
    def destroy(self):
        if self.slash_checker:
            self.slash_checker.destroy()
            self.slash_checker = None
        super(MonsterBrambleBase, self).destroy()
        return
...
class MonsterBramble(MonsterBrambleBase):

    def init_params(self):
        super(MonsterBramble, self).init_params()
        ...
        self.hit_idx = 0
        self.max_hit_dis = self.custom_param.get('max_hit_dis', 10) * NEOX_UNIT_SCALE
