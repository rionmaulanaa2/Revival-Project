...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/battle/Settle/ArmRaceEndSceneUI.py
...
class ArmRaceEndSceneUI(EndSceneUIBase):
    ...
    def on_init_panel(self, settle_dict, reward, teammate_num, teammate_info, total_fighter_num):
        super(ArmRaceEndSceneUI, self).on_init_panel()
    ...
    def _init_settle_data_widget(self):
        ...
        self.settle_data_widget = global_data.uisystem.load_template_create('battle_arms_race/battle_score_detail', parent=self.panel.temp_ffa)
        self.settle_data_widget.setVisible(False)
        self.settle_data_widget.btn_close.setVisible(False)
        rank_data = self.settle_dict.get('group_rank_data', [])
        armrace_max_lv = self.settle_dict.get('armrace_max_lv', 15)
        ...
        for index, widget in enumerate(nd_list.GetAllItem()):
            ...
            for e_index, eid in enumerate(six_ex.keys(dict_data)):
                ...
                is_my = my_eid == eid
                if armrace_lv > armrace_max_lv:
                    widget.lab_score.SetString('LV.Max')
                else:
                    widget.lab_score.SetString(get_text_by_id(17248).format(armrace_lv))
    ...
    def on_finalize_panel(self):
        ...
        if self.bg_music:
            ...
            self.bg_music = None
        super(ArmRaceEndSceneUI, self).on_finalize_panel()
        return
    ...
    def _get_self_name(self):
        if self._is_ob_settle():
            return self.settle_dict.get('ob_data', {}).get('watching_soul_name', '')
        else:
            char_name = global_data.player.logic.ev_g_char_name()
            if char_name:
                return char_name
            return global_data.player.get_name()
    ...
    def on_finish_settle_scene_camera(self):
        ...
        self.name_widget.reset_position_and_show(name_on_role=True)
        self.show()
