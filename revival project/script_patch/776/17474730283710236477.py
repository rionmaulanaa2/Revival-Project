...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityKaixue/ActivityTeacherTest.py
...
class ActivityTeacherTest(ActivityCollect):

    def init_parameters(self):
        super(ActivityTeacherTest, self).init_parameters()
    ...
    def refresh_time(self, parent_task):
        ...
        if left_time > 0:
            if left_time > ONE_HOUR_SECONS:
                self.panel.nd_auto_fit.lab_time.SetString(get_text_by_id(607014).format(get_readable_time_day_hour_minitue(left_time)))
            else:
                self.panel.nd_auto_fit.lab_time.SetString(get_text_by_id(607014).format(get_readable_time(left_time)))
        else:
            close_left_time = 0
            self.panel.nd_auto_fit.lab_time.SetString(get_readable_time(close_left_time))