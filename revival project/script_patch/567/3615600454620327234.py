...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/Activity202201/ActivitySpringFestivalVeteranRecall.py
...
class ActivitySpringFestivalVeteranRecall(ActivityBase):

    def __init__(self, dlg, activity_type):
        super(ActivitySpringFestivalVeteranRecall, self).__init__(dlg, activity_type)
    ...
    def init_parameters(self):
        ...
        c_ui_data = self.activity_conf.get('cUiData')
        self.point_item_no = c_ui_data.get('point_item_no')
        self.cur_points = global_data.player.get_item_num_by_no(self.point_item_no)
        ...
        self.ex_item_list = c_ui_data.get('item_list')
        self.ex_mall_item_list = c_ui_data.get('mall_item_list')
        self.cb_point = c_ui_data.get('cb_point')
        self.gun_type_text_id = c_ui_data.get('gun_type_text_id')
        self.UZI_ID = self.ex_item_list[-1]
        self.UZI_ID_PIC = get_lobby_item_pic_by_item_no(self.UZI_ID)
        self.recall_end_time = c_ui_data.get('recall_end_time')
        self.nd_points = self.panel.nd_content.nd_exchange.nd_reward.lab_exchange.nd_auto_fit.lab_exchange_num
    ...
    def on_init_panel(self):
        super(ActivitySpringFestivalVeteranRecall, self).on_init_panel()
    ...
    def init_ui_events(self):
        self.panel.nd_title.img_title_01.lab_time.SetString(610022)

        @self.panel.nd_title.img_title_01.btn_question.unique_callback()
        def OnClick(_btn, _touch, *args):
            from logic.comsys.common_ui.GameRuleDescUI import GameRuleDescUI
            dlg = GameRuleDescUI()
            rule_text_id = self.activity_conf.get('cRuleTextID')
            rule = int(rule_text_id) if rule_text_id else 610023
            title, rule = 609960, rule
            dlg.set_show_rule(get_text_by_id(title), get_text_by_id(rule))

        self.nd_points.SetString(str(self.cur_points))
        self.panel.nd_content.nd_exchange.nd_reward_01.lab_name.SetString(get_text_by_id(self.gun_type_text_id) + ' ' + get_lobby_item_name(self.UZI_ID))

        @self.panel.nd_content.nd_exchange.nd_reward_01.btn_detail.unique_callback()
        def OnClick(_btn, _touch, *args):
            jump_to_display_detail_by_item_no(self.UZI_ID)

        self.panel.nd_content.nd_player.nd_player_01.nd_recall.lab_tips.SetString(get_text_by_id(609733).format(str(self.cb_point)))
    ...
    def init_item_list(self):
        item_no = self.ex_item_list[-1]
        goods_id = self.ex_mall_item_list[-1]
        ui_item = self.panel.nd_content.nd_exchange.nd_reward_01
        ...
        ui_item.img_reward.SetDisplayFrameByPath('', self.UZI_ID_PIC)
        ui_item.img_reward.SetPosition('50%', '25%')
        rare_icon = item_utils.get_skin_rare_degree_icon(item_no)
        ui_item.nd_kind.bar_level.SetDisplayFrameByPath('', rare_icon)
        ...
        if left_num <= 0:
            ...
            @ui_item.pnl_bg.btn_reward_01.unique_callback()
        ...
        self.nd_item_list.SetInitCount(len(self.ex_item_list) - 1)
        for index, ui_item in enumerate(self.nd_item_list.GetAllItem()):
            ...
            if left_num <= 0:
                ...
                if can_buy_num <= 0:
                    ui_item.img_choose.setVisible(False)
                    ui_item.lab_exchange_need.setVisible(True)
                    ui_item.nd_got.setVisible(False)
                else:
                    ui_item.img_choose.setVisible(True)
                    ui_item.lab_exchange_need.setVisible(True)
                    ui_item.nd_got.setVisible(False)
    ...
    def check_finish(self):
        ...
        if cur_time > self.recall_end_time:
            return True
        return False
    ...
    def update_recall_list(self):
        ...
        for index, ui_item in enumerate(self.nd_recall.list_recall.GetAllItem()):
            ...
            ui_item.lab_name.SetString(role_name)
            self.send_mgs_uids = global_data.player.send_msg_uids
            self.team_dict = global_data.player.lost_role_team_times
    ...
    def update_team_list(self):
        ...
        def OnScrollingCallback():
            self.update_right_btn_state()

        self.nd_team.list_team.OnScrolling = OnScrollingCallback
    ...
    def update_right_btn_state(self):
        all_item = self.nd_team.list_team.GetItemCount()
        show_num = self._get_show_num(self.nd_team.list_team)
        if all_item <= show_num:
            self.nd_team.btn_right.setVisible(False)
            return
        border, indent, ctrl_size, offset = self._get_list_info(self.nd_team.list_team)
        left_idx = (abs(offset.x) - border) / (ctrl_size.width + indent)
        right_idx = left_idx + show_num
        if int(right_idx) == all_item:
            return
        vis = True if int(right_idx) < all_item - 1 else False
        self.nd_team.btn_right.setVisible(vis)

    def _get_list_info(self, list_item):
        border = list_item.GetHorzBorder()
        indent = list_item.GetHorzIndent()
        ctrl_size = list_item.GetCtrlSize()
        offset = list_item.GetContentOffset()
        return (
         border, indent, ctrl_size, offset)

    def _get_show_num(self, list_item):
        border, indent, ctrl_size, _ = self._get_list_info(list_item)
        contentSize = list_item.GetContentSize()
        show_num = (contentSize[0] - border) / (ctrl_size.width + indent)
        return show_num
    ...
    def on_finalize_panel(self):
        ...
        super(ActivitySpringFestivalVeteranRecall, self).on_finalize_panel()