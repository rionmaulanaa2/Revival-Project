...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/Activity202202/ActivityEarlySpringDate.py
...
class ActivityEarlySpringDate(ActivityCollect):

    def __init__(self, dlg, activity_type):
        super(ActivityEarlySpringDate, self).__init__(dlg, activity_type)
        ...
        self._collect_task = act_data.get('collect_task', None)
        return
    ...
    def _init_gun_progress(self):
        book_num = global_data.player.get_item_num_by_no(REWARD_ITEM_ID)
        player = global_data.player
        ...
        book_num = min(book_num, 100)
        self.panel.lab_number.SetString(''.join((get_text_by_id(610676), str(book_num))))
        book_rate = PROG_TO_RATE.get(book_num, book_num)
        self.panel.prog_inside.SetPercentage(book_rate)
    ...
    def refresh_time(self, parent_task):
        ...
        if task_left_time > 0:
            if task_left_time > ONE_HOUR_SECONS:
                self.panel.lab_time.SetString(get_text_by_id(610171).format(get_readable_time_day_hour_minitue(task_left_time)))
            else:
                self.panel.lab_time.SetString(get_text_by_id(610171).format(get_readable_time(task_left_time)))
        else:
            close_left_time = 0
