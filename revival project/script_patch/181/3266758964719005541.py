...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/Activity202201/ActivitySpringFestivalCalendar.py
from __future__ import absolute_import
from logic.comsys.activity.ActivityTemplate import ActivityTemplate
from .ActivitySpringFestivalCalendarMainUI import ActivitySpringFestivalCalendarBase

class ActivitySpringFestivalCalendar(ActivityTemplate):

    def on_init_panel(self):
        self.calendar_widget = ActivitySpringFestivalCalendarTabPage(self.panel)

    def on_finalize_panel(self):
        super(ActivitySpringFestivalCalendar, self).on_finalize_panel()
        self.calendar_widget.on_finalize_panel()


class ActivitySpringFestivalCalendarTabPage(ActivitySpringFestivalCalendarBase):
    SHARE_TASK_ID = '1440544'

    def __init__(self, panel):
        super(ActivitySpringFestivalCalendarTabPage, self).__init__(panel)
        self.process_event(True)
        self.refresh_share()

    def play_show_animation(self):
        super(ActivitySpringFestivalCalendarTabPage, self).play_show_animation()
    ...
    def init_panel(self):
        super(ActivitySpringFestivalCalendarTabPage, self).init_panel()
        ...
        def OnClick(btn, touch):
                ...
                if share_ui and share_ui.is_valid():
                    ...
                    def share_inform_func():
                        if global_data.player:
                            global_data.player.share_activity('activity_20602')
            ...
            if not getattr(self, '_share_content', None):
                from logic.comsys.share.SpringFestivalCalendarShareCreator import SpringFestivalCalendarShareCreator
                self._share_content = SpringFestivalCalendarShareCreator()
    ...
    def jump_from_item(self, idx):
        super(ActivitySpringFestivalCalendarTabPage, self).jump_from_item(idx)
        if idx == 1:
