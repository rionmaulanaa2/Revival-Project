...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/lottery/LotteryArtCollectionWidget.py
...
DEFAULT_ICON_PATH = 'gui/ui_res_2/lottery/btn_lottery_banner_activity.png'
DEFAULT_TXT_COLOR = 16776169
DEFAULT_OUT_LINE = 9048633

class LotteryArtCollectionWidget(LotteryBaseWidget):

    def init_parameters(self):
        super(LotteryArtCollectionWidget, self).init_parameters()
    ...
    def _init_buy_widget(self):
        ...
        def update_price_info_callback(nd, lottery_count):
            if lottery_count == CONTINUAL_LOTTERY_COUNT:
                if global_data.player and global_data.player.determine_lottery_10_try(self.lottery_id, False):
                    self.panel.PlayAnimation('button_tips')
                else:
                    self.panel.StopAnimation('button_tips')
                    self.panel.nd_button_tips.setVisible(False)
    ...
    def _init_preview_widget(self):
        ...
        self.preview_widget = LotteryNewPreviewWidget(self.panel.nd_review, self.panel, self.lottery_id, self.on_change_show_reward, show_callback=show_callback, hide_callback=hide_callback, close_callback=close_callback)
    ...
    def on_finalize_panel(self):
        ...
        super(LotteryArtCollectionWidget, self).on_finalize_panel()
        if not global_data.video_player.is_in_init_state():
            global_data.video_player.stop_video(ignore_cb=True)
