...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityArtCollectionCollect.py
...
class ActivityArtCollectionCollect(ActivityBase):
    TASK_PROGRESS_PERCENTS = None

    def __init__(self, dlg, activity_type):
        super(ActivityArtCollectionCollect, self).__init__(dlg, activity_type)
    ...
    def on_init_panel(self):
        ...
        if self.panel.getChildByName('btn_question'):
    ...
    def init_clothing_item(self, item_config, clothing_item, clothing_id, item_type):
        ...
        item_utils.init_skin_card(clothing_item, clothing_id, True)
        if item_type == lobby_item_type.L_ITEM_TYPE_MECHA_SKIN or item_type == lobby_item_type.L_ITEM_TYPE_MECHA:
            ...
            skin_cfg = confmgr.get('role_info', 'RoleSkin', 'Content', str(clothing_id))
            if skin_cfg:
                item_utils.check_skin_tag(clothing_item.nd_kind, clothing_id)
    ...
    def update_skin(self, is_init=False):
        use_template = self.template_no > 0
        if use_template:
            if self.template_no == 1:
            ...
            elif self.template_no == 2:
                self.panel.list_card.SetInitCount(len(self.ITEM_IMG_CFG))
                for item_no, cfg in six.iteritems(self.ITEM_IMG_CFG):
                    index = cfg[0]
                    item_nd = self.panel.list_card.GetItem(index)
                    collections = global_data.player.get_task_content(self.collect_task_id, 'collections', [])
                    is_show = int(item_no) in collections
                    item_nd.lab_skin_name.SetString(item_utils.get_lobby_item_name(item_no))
                    item_nd.img_skin_on.setVisible(is_show)
                    item_nd.img_skin_off.setVisible(not is_show)
                    if is_init:
                        item_nd.HasAnimation('show') and item_nd.PlayAnimation('show')

            elif self.template_no == 3:
                for item_no, cfg in six.iteritems(self.ITEM_IMG_CFG):
                    list_name, index = cfg
                    item_nd = getattr(self.panel, list_name).GetItem(index)
                    collections = global_data.player.get_task_content(self.collect_task_id, 'collections', [])
                    is_show = int(item_no) in collections
                    item_nd.lab_skin_name.SetString(item_utils.get_lobby_item_name(item_no))
                    item_nd.img_skin_on.setVisible(is_show)
                    item_nd.img_skin_off.setVisible(not is_show)
                    lab_role_name = getattr(item_nd, 'lab_role_name', None)
                    if lab_role_name:
                        name_text = item_utils.get_lobby_item_belong_name(item_no)
                        lab_role_name.SetString(name_text)
                    if is_init:
                        item_nd.PlayAnimation('show')

        task_cur_prog = global_data.player.get_task_prog(self.collect_task_id)
        progress_node = self.panel.progress_bar
        if self.TASK_PROGRESS_PERCENTS:
            percent = self.TASK_PROGRESS_PERCENTS[task_cur_prog]
        else:
        ...
        self.set_btn_get_enable(is_receivable_result)
        self.panel.btn_get.vx_btn and self.panel.btn_get.vx_btn.setVisible(is_receivable_result)
        self.panel.btn_get.lab_text_on.setVisible(is_receivable_result)
        self.panel.btn_get.lab_text_off.setVisible(not is_receivable_result)
        return