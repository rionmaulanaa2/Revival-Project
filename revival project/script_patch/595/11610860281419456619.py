...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/vscene/parts/camera/camera_controller/lottery_cam_ctrl.py
...
class LotteryCamCtrl(CameraCtrl):

    def init(self):
        scene_data = lobby_model_display_utils.get_display_scene_data(lobby_model_display_const.LOTTERY)
        self.on_change_lottery_scene_cam(scene_data, is_init=True)
        self.events_binded = False
        self.process_event(True)

    def process_event(self, flag):
        emgr = global_data.emgr
        e_conf = {'change_lottery_scene_cam': self.on_change_lottery_scene_cam
           }
        if self.events_binded == flag:
            return
    ...
    def on_change_lottery_scene_cam(self, scene_data, is_slerp=False, is_init=False):
        if not is_init:
            ...
            world.get_active_scene().active_camera.fov = fov
            world.get_active_scene().active_camera.aspect = aspect
        key = scene_data.get('cam_key')
