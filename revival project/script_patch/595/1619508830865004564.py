...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/charge_ui/GiftBoxSkinUI.py
...
from logic.gutils import trigger_gift_utils, mall_utils, item_utils
from logic.gutils.template_utils import init_tempate_mall_i_item, init_price_template
...
class GiftBoxSkinUI(BasePanel):
    PANEL_CONFIG_NAME = 'charge/charge_gift'
    ...
    UI_ACTION_EVENT = {'temp_btn_close.btn_back.OnClick': '_on_click_close_btn',
       ...
       'btn_skip.OnClick': '_on_click_skip'
       }
    ...
    def init_parameters(self, gift_info):
        ...
        self._skin_goods_id = trigger_gift_utils.get_skin_goods_id(self._goods_list)
        item_id = mall_utils.get_goods_item_no(self._skin_goods_id)
        self._skin_type = item_utils.get_lobby_item_type(item_id)
        self._closing = False
        self._fly_node = self.panel.nd_ani
    ...
    def on_finalize_panel(self):
        super(GiftBoxSkinUI, self).on_finalize_panel()
    ...
    def init_widget(self):
        for animation_name in trigger_gift_utils.get_gift_ui_animation_names(True, True, self._skin_type, len(self._goods_list)):
    ...
    def _init_left_time_widget(self):
        ...
        if expire_time > tutil.get_server_time():
            self._left_time_widget = LeftTimeCountDownWidget(self.panel, self.panel.lab_time, lambda timestamp: get_text_by_id(608309).format(tutil.get_readable_time(timestamp)))
    ...
    def _init_ui_text(self):
        self.panel.lab_name.SetString(mall_utils.get_goods_name(self._skin_goods_id))
        item_id = mall_utils.get_goods_item_no(self._skin_goods_id)
        self.panel.img_kind.SetDisplayFrameByPath('', item_utils.get_skin_rare_degree_icon(item_id))
        trigger_gift_utils.format_discount_ui_text(self._gift_info.get('discount', 0), self.panel)
        intro_params = [ param for param in self._gift_info.get('intro_params', []) ]
        intro_text = trigger_gift_utils.get_gift_intro_text(intro_params, skin_goods_id=self._skin_goods_id)
        if intro_text:
            self.panel.lab_tips.SetString(intro_text)
            self.panel.lab_tips.setVisible(True)
        else:
            self.panel.lab_tips.setVisible(False)
        if self._skin_type == L_ITEM_TYPE_ROLE_SKIN:
            img_path = trigger_gift_utils.get_role_display_img_path(item_id)
            self.panel.nd_role.setVisible(True)
            self.panel.nd_mech.setVisible(False)
            self.panel.img_role.SetDisplayFrameByPath('', img_path)
            self.panel.img_role2.SetMaskFrameByPath('', img_path)
        elif self._skin_type == L_ITEM_TYPE_MECHA_SKIN:
            img_path = trigger_gift_utils.get_mecha_display_img_path(item_id)
            self.panel.nd_role.setVisible(False)
            self.panel.nd_mech.setVisible(True)
            self.panel.img_mech.SetDisplayFrameByPath('', img_path)
            self.panel.img_mech2.SetMaskFrameByPath('', img_path)

    def _init_gift_goods_items(self):
        goods_list = self._gift_info.get('goods_list')
        if not goods_list:
            return
        if len(goods_list) == 1 and int(self._skin_goods_id) > 0:
            ...
            for i, goods_id in enumerate(goods_list):
                ...
                init_tempate_mall_i_item(ui_item.temp_reward, item_no, item_num=item_num, show_tips=True)

        price_info = {'original_price': self._gift_info.get('original_price', 0),
           'discount_price': self._gift_info.get('discount_price', 0),
           'goods_payment': gconst.SHOP_PAYMENT_YUANBAO
           }
        COLOR = [
         '#SW', '#SR', '#BC']
        init_price_template(price_info, self.panel.temp_price, color=COLOR)
    ...
    def _on_click_skip(self, btn, touch):
        if int(self._skin_goods_id) > 0:
            jump_to_display_detail_by_goods_id(self._skin_goods_id)
    ...
    def _try_close(self):
        ...
        if dst_wpos and motion:
            close_animation_name = trigger_gift_utils.get_gift_ui_animation_names(True, False, self._skin_type, len(self._goods_list))
