...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/gcommon/behavior/PVEMonsterMeleeLogic.py
...
class MonsterMeleeBase(MonsterStateBase):
    ...
    def init_from_dict(self, unit_obj, bdict, sid, info):
        super(MonsterMeleeBase, self).init_from_dict(unit_obj, bdict, sid, info)
        self.init_params()
    ...
    def on_init_complete(self):
        super(MonsterMeleeBase, self).on_init_complete()
    ...
    def reset_hit_range(self):
        ...
        self.slash_checker = SlashChecker(self, self.hit_skill_id, (hit_width, hit_height, hit_depth), self.hit_bone, damage_settlement_always_on=True, only_damage_in_front=not self.damage_on_back)
    ...
    def enter(self, leave_states):
        super(MonsterMeleeBase, self).enter(leave_states)
    ...
    def update(self, dt):
        super(MonsterMeleeBase, self).update(dt)
    ...
    def exit(self, enter_states):
        super(MonsterMeleeBase, self).exit(enter_states)
        if self.is_atking:
            self.is_atking = False
            self.slash_checker.end_check()
        if self.aim_turn:
    ...
    def destroy(self):
        ...
        if self.slash_checker:
            self.slash_checker.destroy()
            self.slash_checker = None
        super(MonsterMeleeBase, self).destroy()
        return
...
class MonsterMelee(MonsterMeleeBase):

    def init_params(self):
        super(MonsterMelee, self).init_params()
        ...
        self.damage_on_back = self.custom_param.get('damage_on_back', False)
        return
