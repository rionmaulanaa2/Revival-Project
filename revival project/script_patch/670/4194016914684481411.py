...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityLianliankan.py
...
class ActivityLianliankan(ActivityTemplate):

    def on_init_panel(self):
        self.activity_conf = confmgr.get('c_activity_config', self._activity_type)
        ui_data = self.activity_conf.get('cUiData')
        self.ultimate_reward = ui_data['ultimate_reward']
        self.chip_item_id = ui_data['chip_item_id']
        self.panel.lab_reward_name.SetString(get_lobby_item_name(self.ultimate_reward))
        self.panel.lab_time.SetString(get_time_string(fmt='%Y.%m.%d', ts=self.activity_conf['cBeginTime']) + '-' + get_time_string(fmt='%Y.%m.%d', ts=self.activity_conf['cEndTime']))
    ...
    def update_final_reward(self):
        ...
        self.panel.bar_red.setVisible(ultimate_reward_gain)
        self.panel.bar_gray.setVisible(not ultimate_reward_gain)
    ...
    def update_task_list(self, *args):
        ...
        if random_task_list is None:
            ...
            for idx, (task_state, child_task) in enumerate(children_task_list):
                ...
                item.lab_schedule.SetString('%s/%s' % (global_data.player.get_task_prog(child_task), get_total_prog(child_task)))
    ...
    def refresh_time(self):
        ...
        self.panel.lab_remaining_time_2.SetString(get_simply_time(task_left_time))

    def on_finalize_panel(self):
        super(ActivityLianliankan, self).on_finalize_panel()
