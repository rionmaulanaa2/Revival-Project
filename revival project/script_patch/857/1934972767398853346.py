...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivitySchoolOpenLogin.py
...
PIC0 = 'gui/ui_res_2/activity/activity_202109/schoolday_seven/pnl1/{}.png'
PIC1 = 'gui/ui_res_2/activity/activity_202109/schoolday_seven/pnl2/{}.png'
PIC2 = 'gui/ui_res_2/activity/activity_202109/schoolday_seven/pnl3/{}.png'

class ActivitySchoolOpenLogin(ActivityCollect):

    def init_parameters(self):
        super(ActivitySchoolOpenLogin, self).init_parameters()
    ...
    def refresh_list(self):
        ...
        for i, task_id in enumerate(self._children_tasks):
            ...
            day_text_1 = '<fontname="gui/fonts/g93_en.otf"><size=34><color=0x5A789DFF>{}</color></size></fontname>'.format(i + 1)
            day_text_2 = '<fontname="gui/fonts/g93_en.otf"><size=34><color=0XFFFFFFFF>{}</color></size></fontname>'.format(i + 1)

            def check_btn(btn=btn_bar):
                has_rewarded = global_data.player.has_receive_reward(task_id)
                pic_index = i + 1
                if has_rewarded:
                    item_widget.lab_kaiqi.SetString('<color=0XFFFFFFFF>{}</color>'.format(get_text_by_id(604010)))
                    item_widget.lab_sign.SetString('<color=0XFFFFFFFF>{}</color>'.format(item_name))
                    item_widget.img_blue_check.setVisible(True)
                    item_widget.img_cover.setVisible(True)
                    btn.SetFrames('', [PIC0.format(pic_index), PIC0.format(pic_index), PIC2.format(pic_index)], False, None)
                    btn.SetEnable(False)
                    item_widget.lab_days.SetString(get_text_by_id(609889).format(day_text_2))
                elif cur_times < total_times:
                    item_widget.lab_kaiqi.SetString('<color=0X184B89FF>{}</color>'.format(get_text_by_id(604026)))
                    item_widget.lab_sign.SetString('<color=0X316978FF>{}</color>'.format(item_name))
                    item_widget.img_blue_check.setVisible(False)
                    item_widget.img_cover.setVisible(False)
                    btn.SetFrames('', [PIC0.format(pic_index), PIC0.format(pic_index), PIC0.format(pic_index)], False, None)
                    btn.SetEnable(False)
                    item_widget.lab_days.SetString(get_text_by_id(610636).format(day_text_1))
                else:
                    item_widget.lab_kaiqi.SetString('<color=0XFFFFFFFF>{}</color>'.format(get_text_by_id(607961)))
                    item_widget.lab_sign.SetString('<color=0XFFFFFFFF>{}</color>'.format(item_name))
                    item_widget.img_blue_check.setVisible(False)
                    item_widget.img_cover.setVisible(False)
                    btn.SetFrames('', [PIC0.format(pic_index), PIC0.format(pic_index), PIC1.format(pic_index)], False, None)
                    btn.SetEnable(False)
                    item_widget.lab_days.SetString(get_text_by_id(609889).format(day_text_2))
                return
    ...
    def _init_get_all(self):
        ...
        if player.has_unreceived_task_reward(self._task_id):
            self.panel.btn_get.SetText(get_text_by_id(607962))
            ...
            if player.has_receive_all_rewards(self._task_id):
                self.panel.btn_get.SetText(get_text_by_id(604029))
            else:
