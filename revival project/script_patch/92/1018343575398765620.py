...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/Activity202203/ActivityQingmingDayLogin.py
...
import cc
PIC0 = 'gui/ui_res_2/activity/activity_202203/qingming_login/btn_qingming_0.png'
PIC1 = 'gui/ui_res_2/activity/activity_202203/qingming_login/btn_qingming_2.png'

class ActivityQingmingDayLogin(ActivityCollect):

    def init_parameters(self):
        super(ActivityQingmingDayLogin, self).init_parameters()
    ...
    def refresh_list(self):
        ...
        for idx, task_id in enumerate(self._children_tasks):
            ...
            if is_received:
                ...
                item_widget.lab_kaiqi.SetString('<color=0X5AA492FF>{}</color>'.format(get_text_by_id(604010)))
                btn_bar.SetFrames('', [PIC0, PIC0, PIC0])
                item_widget.temp_items.lab_quantity.SetColor(16776884)
                item_widget.lab_sign.SetColor(2711167)
                item_widget.lab_days.SetString('<color=0x1F7286FF>' + get_text_by_id(610373).format(day) + '</color>')
            elif can_receive:
                item_widget.lab_kaiqi.SetString('<color=0XA4825AFF>{}</color>'.format(get_text_by_id(607961)))
                btn_bar.SetFrames('', [PIC1, PIC1, PIC1])
                item_widget.img_blue_check.setVisible(False)
                item_widget.temp_items.lab_quantity.SetColor(16776938)
                item_widget.lab_sign.SetColor(8345641)
                item_widget.lab_days.SetString('<color=0x967851FF>' + get_text_by_id(610373).format(day) + '</color>')
            else:
                item_widget.img_blue_check.setVisible(False)
                item_widget.lab_kaiqi.SetString('<color=0X5AA492FF>{}</color>'.format(get_text_by_id(604026)))
                btn_bar.SetFrames('', [PIC0, PIC0, PIC0])
                item_widget.temp_items.lab_quantity.SetColor(16776884)
                item_widget.lab_sign.SetColor(2711167)
                item_widget.lab_days.SetString('<color=0x1F7286FF>' + get_text_by_id(610373).format(day) + '</color>')
