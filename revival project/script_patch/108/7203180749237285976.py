...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/items_book_ui/GuangmuGetUseBuyWidget.py
...
class GuangmuGetUseBuyWidget(object):
    ...
    def update_buy_widget(self, can_buy):
        if self.buy_btn:
            self.buy_btn.setVisible(bool(can_buy))
        if self.goods_id:
    ...
    def refresh_widget(self):
        ...
        if global_data.player:
            cur_guangmu_no = global_data.player.get_selected_guangmu()
        using = cur_guangmu_no == int(self.item_no)
        self.cancel_btn and self.cancel_btn.setVisible(using)
        can_use = item_can_use and not using
        self.use_btn and self.use_btn.setVisible(can_use)
    ...
    def on_click_use_btn(self, *args):
        if not self.item_no:
            return
        global_data.player and global_data.player.set_selected_guangmu(self.item_no)

    def on_click_cancel_btn(self, *args):
        if not global_data.player:
            return
        global_data.player.set_selected_guangmu(0)
