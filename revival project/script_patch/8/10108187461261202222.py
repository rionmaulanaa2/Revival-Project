...
# Embedded file name: /Users/netease/Documents/work/battlegrounds/gameplay/releases/rel_current/tools/patch/temp/script/logic/comsys/activity/ActivityHalloween/ActivityHalloweenTrainExchange.py
...
class ActivityHalloweenTrainExchange(ActivityExchange):

    def __init__(self, dlg, activity_type):
        super(ActivityHalloweenTrainExchange, self).__init__(dlg, activity_type)
    ...
    def on_finalize_panel(self):
        super(ActivityHalloweenTrainExchange, self).on_finalize_panel()
        self.sub_widget = None
        return
    ...
    def set_show(self, show, is_init=False):
        super(ActivityHalloweenTrainExchange, self).set_show(show, is_init)
    ...
    def reorder_task_list(self, tasks):
        ...
        def cmp_func(task_id_a, task_id_b):
            ...
            prices = mall_utils.get_mall_item_price(goods_id_a, pick_list=['item', 'yuanbao'])
            price_info = prices[0]
            ...
            prices = mall_utils.get_mall_item_price(goods_id_b, pick_list=['item', 'yuanbao'])
            price_info = prices[0]
            goods_payment = price_info.get('goods_payment')
            cost_item_no = mall_utils.get_payment_item_no(goods_payment)
    ...
    def show_list(self):
        ...
        for i, task_id in enumerate(children_tasks):
            ...
            if not goods_id:
                return
            prices = mall_utils.get_mall_item_price(goods_id, pick_list=['item', 'yuanbao'])
            if not prices:
                return
            price_info = prices[0]
            goods_payment = price_info.get('goods_payment')
    ...
    def _refresh_goods(self):
        conf = confmgr.get('c_activity_config', self._activity_type)
        goods_list = conf.get('cUiData', {}).get('goods_list', [])
        for i, goods_id in enumerate(goods_list):
            goods_id = str(goods_id)
            item_node = getattr(self.panel, 'img_reward_bar_0%s' % (i + 1))
            if not item_node:
                continue
            item_no = str(mall_utils.get_goods_item_no(goods_id))
            max_buy_num_all = mall_utils.get_goods_limit_num_all(goods_id)
            buy_num = global_data.player.get_buy_num_all(goods_id)
            init_tempate_mall_i_item(item_node.temp_item, item_no, show_rare_degree=False)
            item_node.lab_item_name.SetString(item_utils.get_lobby_item_name(item_no))
            if max_buy_num_all:
                item_node.lab_num.SetString(get_text_by_id(907118).format(max_buy_num_all - buy_num))
            else:
                item_node.lab_num.SetString('')
            item_node.temp_s.bar_level.SetDisplayFrameByPath('', item_utils.get_skin_rare_degree_icon(item_no))